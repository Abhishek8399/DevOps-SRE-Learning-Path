...
The current repository is an early implementation. Eighteen routed lesson identities are available across Volumes 00 through 03: five established typed lessons and thirteen schema-backed `substantive-draft` lessons (`LES-0006` through `LES-0018`). The structured checkpoint contains 39 assessments and 104 references; its automated content, schema, reader, lint, type, build, and 27-route production audits pass. The ten guarded labs for `LES-0009` through `LES-0018` also pass their Ubuntu verifiers, ShellCheck for all 20 `lab.sh`/`verify.sh` scripts, and root-refusal checks. These are project-validation results, not accepted chapters or learner evidence. Formal technical and instructional review, manual browser QA, independently reviewed learner transfer and delayed recall, and representative real-environment exercises remain open. Most curriculum domains remain planned or not started. `CONTENT_MATRIX.md` is the authoritative coverage audit; `progress/ledger.md` remains the authoritative learner-evidence record.
