...
|---|---|---|
| [Local learning cockpit](../learning-cockpit/README.md) | The uncommitted canonical worktree exposes twenty-six routed lesson identities across Volumes 00 through 04: five legacy and twenty-one structured; artifact publication is not learner competency evidence | The generated corpus validates at 21 lessons, 63 assessments, and 172 references; automated integration passes, while real-browser interaction remains unclaimed |
| [Reliability Atlas reader](../learning-cockpit/README.md) | Routed, Ubuntu-first reader exposes twenty-six lessons across five populated volumes, including twenty-one schema-backed lessons; artifact publication is not learner competency evidence | `LES-0026` resolves at `/book/reliability/observability-foundations`; the 36-route loopback audit and 37-target internal-link crawl pass, while no reading state, mentor-operated verifier, local teaching model, or published artifact becomes mastery or provider-acceptance evidence |
| [Offline search and device-local reading desk](../learning-cockpit/README.md) | Twenty-six-result cross-volume catalog/search, bookmarks, recent/resume history, private reading markers, command-copy feedback, and additive state integration are present; reading state and artifact publication are not learner competency evidence | Canonical `LES-0001` through `LES-0026` search, the additive eighteen-lesson state migration, and all twenty-one answer-isolation contracts pass pure reader tests; real-browser persistence, keyboard, and clipboard checks remain unavailable |
| [Structured content publishing contract](../book/schema/README.md) | Strict lesson, assessment, reference, identity, path, safety, and relationship validation covers twenty-one structured lessons, 63 assessments—forty-two complete-answer records and twenty-one answer-isolated transfers—and 172 references; artifact publication is not learner competency evidence | The IDs continue through `LES-0026`, `ASM-0063`, and `REF-0172`; the next unused IDs are `LES-0027`, `ASM-0064`, and `REF-0173`. Automated integration passes; feature commit, push, and exact remote parity remain pending |
...
| [LES-0025 — CI platform operations](../book/volumes/03-engineering-delivery/LES-0025-ci-platform-operations/lesson.md) | Schema-backed `substantive-draft` as canonical `LES-0025` / alias `V03-L10` / `CI-002` at `/book/engineering/ci-platform-operations`, with three assessments, eleven official references, and a bounded dual-engine local lab; artifact publication and mentor execution are not learner competency evidence | Commit `a80ca72`; exact content, registry, reader, schema, lint, type, build, Ubuntu verifier, root-refusal, cleanup, HTTP, hygiene, and remote-parity gates pass. The engines still do not execute or certify GitHub Actions, GitLab CI/CD, Jenkins, or Azure Pipelines; formal acceptance, provider-specific evidence, learner execution, independently reviewed transfer, delayed recall, and mastery remain pending |
| [LES-0026 — Observability foundations](../book/volumes/04-reliability-operations/LES-0026-observability-foundations/lesson.md) | Schema-backed `substantive-draft` in the uncommitted canonical worktree as `LES-0026` / alias `V04-L01` / `OBS-001` at `/book/reliability/observability-foundations`, with three assessments, nine official or primary references, and a bounded local five-signal lab; artifact publication and mentor execution are not learner competency evidence | Draft checkpoint `6628575` preserves the reviewed candidate. Canonical content, registry, schema, reader, lint, typecheck, build, Ubuntu verifier, HTTP, asset, link, and hygiene gates pass; real-browser interaction is unavailable, and feature commit, push, and parity remain pending. The fixture proves only its encoded local model; it is not a representative instrumented service, OpenTelemetry SDK or Collector, Prometheus, Grafana, continuous profiler, vendor backend, production behavior, causality proof, learner transfer, formal acceptance, delayed recall, or mastery |
