...
|---|---|---|
| [Local learning cockpit](../learning-cockpit/README.md) | Volume 1 contains six detailed Linux lessons; artifact publication is not learner competency evidence | Commit `24201bb`; loopback verification returned HTTP 200 for all 12 local routes and confirmed one H1 per page, six searchable lesson identities, and no external executable assets |
| [Systems Reliability Field Manual reader](../learning-cockpit/README.md) | Routed, Ubuntu-first reader now serves all six lessons, including the first schema-backed lesson; artifact publication is not learner competency evidence | Commit `24201bb`; 17 reader tests, lint, typecheck, production build, and the 12-route loopback smoke test passed; invalid routes returned 404 and the server remained bound to `127.0.0.1` |
| [Offline search and device-local reading desk](../learning-cockpit/README.md) | Six-result offline search, bookmarks, recent/resume history, private reading markers, and command-copy feedback implemented; reading state and artifact publication are not learner competency evidence | Commit `24201bb`; the production route harness found all six lesson IDs through search while preserving the five legacy lesson routes and device-local state IDs |
| [Structured content publishing contract](../book/schema/README.md) | Strict lesson, assessment, reference, identity, path, safety, and relationship validation is live with one production lesson; artifact publication is not learner competency evidence | Commit `24201bb`; the 36-case schema suite passed 35 cases with one Windows `EPERM` symlink skip, the 17-case reader suite passed, and the live corpus validates 1 lesson, 3 assessments, 8 references, and 5 legacy reservations |
