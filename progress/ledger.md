...
|---|---|---|
| [Local learning cockpit](../learning-cockpit/README.md) | Volumes 00 and 01 contain seven detailed lessons; artifact publication is not learner competency evidence | Commit `817bb60`; loopback verification returned HTTP 200 for all 14 local routes and confirmed one H1 per page, seven searchable lesson identities, and no external executable assets |
| [Systems Reliability Field Manual reader](../learning-cockpit/README.md) | Routed, Ubuntu-first reader serves seven lessons across two volumes, including two schema-backed lessons; artifact publication is not learner competency evidence | Commit `817bb60`; all 20 reader tests, lint, typecheck, content validation, production build, and the 14-route loopback smoke test passed; both invalid lesson routes returned 404 |
| [Offline search and device-local reading desk](../learning-cockpit/README.md) | Seven-result cross-volume offline search, bookmarks, recent/resume history, private reading markers, and command-copy feedback implemented; reading state and artifact publication are not learner competency evidence | Commit `817bb60`; the reader suite and production route harness found all seven lesson identities while preserving all legacy routes and device-local state IDs |
| [Structured content publishing contract](../book/schema/README.md) | Strict lesson, assessment, reference, identity, path, safety, and relationship validation is live with two structured lessons; artifact publication is not learner competency evidence | Commit `817bb60`; the 39-case schema suite passed 38 cases with one Windows `EPERM` symlink skip, the 20-case reader suite passed, and the corpus validates 2 lessons, 6 assessments, 16 references, and 5 legacy reservations |
