...
|---|---|---|
| [Local learning cockpit](../learning-cockpit/README.md) | The worktree exposes eight detailed lessons across Volumes 00 and 01; artifact publication is not learner competency evidence | Current project audit passes 15 declared routes with one `h1`, two invalid-route 404s, 15 discovered internal links, LES-0008 payload checks, eight canonical search IDs, and exact loopback cleanup; browser interaction remains blocked and unclaimed |
| [Reliability Atlas reader](../learning-cockpit/README.md) | Routed, Ubuntu-first reader serves eight lessons across two volumes, including three schema-backed lessons; artifact publication is not learner competency evidence | All 21 reader tests, lint, typecheck, content validation, and build pass; the build retains only known warning classes, and no reading state becomes mastery evidence |
| [Offline search and device-local reading desk](../learning-cockpit/README.md) | Eight-result cross-volume catalog/search, bookmarks, recent/resume history, private reading markers, command-copy feedback, and additive state integration are authored; reading state and artifact publication are not learner competency evidence | Canonical `LES-0001` through `LES-0008` search and the seven-to-eight state migration pass automated tests; real-browser persistence, keyboard, and clipboard checks remain blocked |
| [Structured content publishing contract](../book/schema/README.md) | Strict lesson, assessment, reference, identity, path, safety, and relationship validation covers three structured lessons, nine assessments, and 24 references; artifact publication is not learner competency evidence | Content validation passes with all 46 requirements and 107 curriculum IDs; 38 of 39 schema cases pass with one Windows `EPERM` symlink skip |
