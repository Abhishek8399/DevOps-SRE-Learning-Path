...
const mockQuestionCores = [
  ...
  {
    id: "sre-dns-tls-partial-outage",
    role: "SRE",
    areas: ["Networking", "Incident response"],
    prompt: "Only some clients receive TLS failures for a public API after a certificate rotation. Server health and aggregate success remain green. Lead the diagnosis and safe recovery.",
    evaluator: "Whether you trace client cohorts through DNS, address families, edge termination, certificate chains, clocks and rollout identity without replacing certificates blindly.",
    strongAnswer: "Define the failing clients, regions, resolver answers, address family, SNI hostname, time window and exact TLS error. Compare successful and failing paths from outside the service: DNS records and TTLs, IPv4 versus IPv6 endpoints, edge or load-balancer listener identity, served leaf/intermediate chain, SANs, validity times and trust-store differences. Map the rotation rollout and preserve both old and new certificate identities. If evidence isolates one edge population or incomplete chain, use the smallest authorized rollback or complete the missing deployment; do not change DNS and certificates simultaneously. Verify with representative affected clients and an end-to-end authenticated API operation, then add expiry, chain, cohort and rollout-consistency monitoring.",
    followUps: ["Why can a browser succeed while an older runtime fails the same hostname?", "What evidence separates stale DNS from inconsistent certificate deployment?"],
  },
  {
    id: "platform-multitenant-saturation",
    role: "Platform engineer",
    areas: ["Platform design", "Reliability"],
    prompt: "One tenant's batch workload saturates a shared Kubernetes platform and latency rises for unrelated services. Design immediate containment and a durable platform correction.",
    evaluator: "Whether you reason across requests, limits, quotas, priority, topology and downstream capacity while preserving tenant and user boundaries.",
    strongAnswer: "Name the affected user journeys, tenants, namespaces, nodes and shared dependencies. Correlate workload start time with CPU throttling, memory pressure, scheduler events, eviction, network or storage saturation and dependency queues. Apply the smallest authorized containment, such as pausing the identified batch controller or limiting its concurrency, with an owner and abort condition; avoid broad eviction that creates more load. Verify unrelated user journeys and resource recovery. Then define supported workload classes, enforce requests and quotas, use priority and preemption deliberately, isolate sensitive pools or dependencies where justified, expose tenant consumption, test overload behavior and establish a capacity/review process. Limits alone are not a complete fairness policy.",
    followUps: ["When can CPU limits make latency worse?", "Which workloads justify separate node pools or clusters rather than namespace controls?"],
  },
  {
    id: "devops-runner-parity",
    role: "DevOps engineer",
    areas: ["Delivery security", "Incident response"],
    prompt: "A build passes on laptops but fails intermittently on CI runners, and retrying usually works. How do you investigate without normalizing flaky retries?",
    evaluator: "Whether you compare execution contracts, preserve the first failure and separate cache, dependency, concurrency, environment and test-order causes.",
    strongAnswer: "Preserve the original failing job, logs, runner image and identity, source revision, dependency lock, environment, test seed, shard, cache keys and artifact checksums. Compare the local and CI execution contracts: OS and architecture, tool versions, filesystem case and permissions, locale/timezone, network availability, CPU/memory, parallelism and clean-state assumptions. Reproduce with the same immutable runner image and cache disabled, then vary one suspected boundary at a time. Quarantine only with an owner, expiry and visible signal; do not hide failure with unconditional retry. Fix the violated contract, pin or isolate inputs, and prove repeated clean runs plus a failure test that would catch recurrence.",
    followUps: ["When is retry a valid resilience mechanism rather than test concealment?", "How can a cache key produce a green but incorrect build?"],
  },
  {
    id: "cloud-terraform-state-lock",
    role: "Cloud engineer",
    areas: ["Platform design", "Delivery security"],
    prompt: "A Terraform pipeline was interrupted. The remote state is locked, a cloud resource exists, and the next plan wants to create it again. What do you do?",
    evaluator: "Whether you reconcile configuration, state and real infrastructure before touching the lock or applying another plan.",
    strongAnswer: "Stop concurrent applies and capture the workspace, backend, lock holder, run identity, configuration revision, saved plan if any, state version and target resource identity without printing secrets. Determine whether the original process is truly dead and whether the lock is stale; force-unlock only the exact reviewed lock after proving no writer remains. Read the provider object and state address, then choose the least-changing reconciliation: allow state refresh, import/adopt the existing object under the intended address, restore a proven state version, or remove an invalid partial object if explicitly authorized. Generate and review a new plan; never reuse an old plan after state changes. Verify convergence with a zero-change plan and service-level checks, then improve locking, interruption handling and run serialization.",
    followUps: ["Why is deleting the lock file first unsafe?", "When would importing the resource be worse than removing and recreating it?"],
  },
  {
    id: "infrastructure-enospc-mount",
    role: "Infrastructure engineer",
    areas: ["Incident response", "Reliability"],
    prompt: "An application reports `No space left on device`, but the VM's root filesystem has plenty of free gigabytes. Walk through diagnosis and remediation.",
    evaluator: "Whether you scope evidence to the exact failing path and distinguish blocks, inodes, quotas, mounts and deleted-open files before deleting anything.",
    strongAnswer: "Start with the exact operation and path, then resolve its mount and filesystem rather than reading only `/`. Use `findmnt -T` or an equivalent mount view, `df -hT` for block capacity and `df -i` for inode capacity on that path. Check user/project quota where applicable and compare visible usage with filesystem allocation; deleted-but-open files can retain blocks until the owning process closes them. If inodes are exhausted, find which approved subtree contains many objects using bounded same-filesystem counts, identify ownership and retention policy, and remove only authorized disposable data. If blocks are exhausted, locate allocated data safely and prefer application-supported rotation or retention. Prove restored headroom and retry the original operation, then correct retention, quotas, alerts or file-creation behavior.",
    followUps: ["Why can deleting one large file fail to free space immediately?", "What evidence must you obtain before deleting millions of small cache files?"],
  },
  {
    id: "data-schema-backfill",
    role: "Data platform engineer",
    areas: ["Data systems", "Reliability"],
    prompt: "A producer changed an event field from seconds to milliseconds. Some consumers silently accepted the values and a large backfill is requested. Design safe recovery.",
    evaluator: "Whether you establish schema and unit identity, bound corrupted intervals, protect downstream effects and make replay observable and idempotent.",
    strongAnswer: "Stop or quarantine the incompatible flow according to the data contract and record producer version, schema identity, affected topics/partitions, offsets and time interval. Identify every consumer and materialized effect; permissive parsing means process success is not semantic correctness. Create a versioned compatibility rule or corrected field rather than guessing values from magnitude alone. Reprocess a small isolated interval into a shadow destination, reconcile counts, units, keys and business invariants against an independent source, and prove sink idempotency or use a new versioned output before scaling. Rate-limit the backfill against broker, compute, database and user-facing capacity, checkpoint progress, expose lag/errors and retain a stop/rollback boundary. Verify downstream user and financial outcomes before retiring old data.",
    followUps: ["Why is value-magnitude detection an unsafe permanent schema contract?", "How do you prevent a backfill from starving real-time traffic?"],
  },
  {
    id: "architect-disaster-recovery",
    role: "Systems architect",
    areas: ["System design", "Reliability"],
    prompt: "Design disaster recovery for a payment platform with a 15-minute RPO and 60-minute RTO. The business proposes nightly backups and DNS failover. Evaluate the design.",
    evaluator: "Whether you translate RPO/RTO into per-state mechanisms, dependencies, authority, reconstruction order and tested user outcomes.",
...
const mockQuestionGuidance = {
  ...
  "sre-dns-tls-partial-outage": {
    topic: "DNS and TLS cohort failures",
    difficulty: "Expert",
    expectedLevel: "Senior",
    weakAnswerWarnings: ["Reissues or replaces certificates without identifying which endpoint and chain failed.", "Uses server health as proof that all client DNS, address-family and trust paths are healthy."],
    deeperExplanation: "Partial TLS failures are often path- or client-specific. DNS caching, dual-stack endpoints, distributed edge configuration, intermediate delivery, SNI and trust-store age can create different handshakes for the same public name.",
    productionExample: "An IPv4 edge may serve the complete new chain while an IPv6 listener serves a leaf without its intermediate, so modern browsers recover but an older runtime fails validation.",
  },
  "platform-multitenant-saturation": {
    topic: "Multi-tenant platform fairness and overload",
    difficulty: "Expert",
    expectedLevel: "Senior",
    weakAnswerWarnings: ["Evicts or restarts workloads broadly before identifying the shared saturated resource.", "Treats requests, limits, or namespace quotas alone as a complete isolation design."],
    deeperExplanation: "Tenancy is enforced across multiple shared queues and authorities. Scheduler placement, kernel throttling, storage/network contention and downstream connection pools can cross namespace boundaries even when resource objects are valid.",
    productionExample: "An unconstrained batch fan-out can exhaust a shared database pool and disk queue, slowing unrelated services despite separate namespaces and apparently available cluster CPU.",
  },
  "devops-runner-parity": {
    topic: "Local-to-CI execution parity and flakiness",
    difficulty: "Advanced",
    expectedLevel: "Senior",
    weakAnswerWarnings: ["Adds retries until the pipeline becomes green without retaining the first failure.", "Says it works locally without comparing immutable environment, cache and concurrency inputs."],
    deeperExplanation: "A build result is a function of source plus every execution input. Intermittency usually exposes an undeclared dependency, race, time/order assumption, shared state or cache identity defect; clean repetition is evidence only after that boundary is controlled.",
    productionExample: "Parallel tests writing the same temporary filename can pass on a developer laptop and race on a larger CI runner; a unique test-owned directory makes ownership explicit.",
  },
  "cloud-terraform-state-lock": {
    topic: "Terraform interrupted-apply reconciliation",
    difficulty: "Expert",
    expectedLevel: "Senior",
    weakAnswerWarnings: ["Force-unlocks immediately without proving the original state writer is gone.", "Runs apply again or deletes the real object before reconciling configuration, state address and provider identity."],
    deeperExplanation: "Terraform coordinates three authorities: configuration intent, recorded state and provider reality. An interrupted operation can change one or two without the third, so safe recovery minimizes additional mutation until their identities are reconciled.",
    productionExample: "A provider can create a load balancer before the pipeline loses its response; state lacks the ID, and a blind retry attempts a duplicate until import or explicit cleanup restores one authority.",
  },
  "infrastructure-enospc-mount": {
    topic: "Linux ENOSPC on the exact filesystem",
    difficulty: "Advanced",
    expectedLevel: "Mid-level",
    weakAnswerWarnings: ["Checks only root filesystem free space and concludes the error is false.", "Deletes broad cache or log paths without mount, ownership, retention and open-file evidence."],
    deeperExplanation: "ENOSPC means the filesystem or enforced allocation domain could not satisfy an allocation. Data blocks, inode records, quotas and mount-specific limits are independent; the failing path selects which filesystem matters.",
    productionExample: "A small `/var` tmpfs can exhaust its fixed inode pool with tiny files while `/` retains hundreds of gigabytes, producing the same write error for an application cache.",
  },
  "data-schema-backfill": {
    topic: "Schema evolution and bounded backfill",
    difficulty: "Expert",
    expectedLevel: "Senior",
    weakAnswerWarnings: ["Multiplies or divides every value based only on size without version or interval evidence.", "Launches an unbounded replay before proving sink identity, capacity protection and reconciliation."],
    deeperExplanation: "Schema compatibility includes meaning and units, not only wire type. A consumer that parses a number successfully can corrupt business state, making semantic validation, lineage and versioned contracts essential recovery inputs.",
    productionExample: "A timestamp still fits a 64-bit integer after changing units, but windowing and retention logic shift by three orders of magnitude while jobs remain technically successful.",
  },
  "architect-disaster-recovery": {
    topic: "RPO/RTO-driven disaster recovery",
    difficulty: "Expert",
    expectedLevel: "Architect",
