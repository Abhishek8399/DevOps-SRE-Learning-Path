...
MODEL="${SCRIPT_DIR}/fixtures/prometheus_model.py"
SOURCE_SCENARIO="${SCRIPT_DIR}/fixtures/scenario.json"
CURRENT_UID="$(id -u)"
STATE_PARENT="/tmp"
STATE_NAME="reliability-atlas-les0028-${CURRENT_UID}"
...
validate_state() {
  [[ "${STATE_DIR}" == "/tmp/reliability-atlas-les0028-${CURRENT_UID}" ]] || { printf 'refused=true reason=state-name-invalid\n' >&2; exit 78; }
...
setup() {
  ...
  python3 -c 'import json,sys; json.dump({"schemaVersion":1,"lessonId":"LES-0028","uid":int(sys.argv[1]),"statePath":sys.argv[2],"caseId":"prometheus-reasoning-v1"},open(sys.argv[3],"w",encoding="utf-8"),indent=2); open(sys.argv[3],"a",encoding="utf-8").write("\n")' "${CURRENT_UID}" "${STATE_DIR}" "${candidate}/manifest.json"
...
status() {
  ...
  printf 'state=ready path=%s case=prometheus-reasoning-v1 results=%s runtime=deterministic-model-only\n' "${STATE_DIR}" "${result_count}"
}
...
cleanup() {
  ...
  [[ "${resolved}" == "/tmp/reliability-atlas-les0028-${CURRENT_UID}" ]] || { printf 'refused=true reason=cleanup-realpath-invalid\n' >&2; exit 78; }
