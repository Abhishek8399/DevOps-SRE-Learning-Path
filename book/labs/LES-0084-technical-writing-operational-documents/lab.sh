...
SENTINEL="$STATE/.les0084"
die(){ printf 'lab=fail reason=%s\n' "$1" >&2; exit 1; }

guard(){
  [[ "$(id -u)" -ne 0 ]] || die root-refused
  command -v python3 >/dev/null 2>&1 || die python3-missing
  [[ -z "${AWS_PROFILE:-}${AWS_ACCESS_KEY_ID:-}${AWS_SECRET_ACCESS_KEY:-}${AWS_SESSION_TOKEN:-}${AZURE_CLIENT_ID:-}${AZURE_CLIENT_SECRET:-}${GOOGLE_APPLICATION_CREDENTIALS:-}${KUBECONFIG:-}${DOCKER_HOST:-}${DOCS_PUBLISH_TOKEN:-}${INCIDENT_ROOM_URI:-}${PRODUCTION_ENDPOINT:-}" ]] || die credential-or-runtime-authority
  [[ ! -L "$STATE" ]] || die state-symlink
  if [[ -e "$STATE" ]]; then
    [[ -d "$STATE" && -O "$STATE" && -f "$SENTINEL" && ! -L "$SENTINEL" ]] || die state-ownership
    [[ "$(<"$SENTINEL")" == "LES-0084:$(id -u)" ]] || die sentinel
  fi
}
...
inventory_state(){
  ...
  while IFS= read -r -d '' item; do
    relative="${item#"$STATE"/}"
    case "$relative" in .les0084|cases.json|packet.json) ;; *) die "unknown-artifact:$relative" ;; esac
...
doctor(){
  ...
  for action in roadmap claims timeline runbook freshness audiences; do python3 "$MODEL" "$action" "$PACKET" >/dev/null; done
  printf 'doctor=pass network=none user=%s publish_calls=none runtime_calls=none\n' "$(id -u)"
}

setup(){
  guard
  [[ ! -e "$STATE" ]] || die state-exists
  ( umask 077; mkdir -- "$STATE"; printf 'LES-0084:%s\n' "$(id -u)" >"$SENTINEL"; cp -- "$CASES" "$STATE/cases.json"; cp -- "$PACKET" "$STATE/packet.json" )
...
status(){
  ...
  printf 'status=ready cases=%s packet_id=%s state=%s publish_calls=none runtime_calls=none\n' "$count" "$packet_id" "$STATE"
}
...
case "${1:-help}" in
  ...
  roadmap|claims|timeline|runbook|freshness|audiences) [[ $# -eq 1 ]] || die arguments; run_model "$1" "$STATE/packet.json" ;;
  ...
  help|-h|--help) printf '%s\n' 'Usage: bash lab.sh doctor|setup|status|list|show CASE|evaluate CASE|evaluate-all|roadmap|claims|timeline|runbook|freshness|audiences|cleanup' ;;
