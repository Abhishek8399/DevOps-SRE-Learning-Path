...
P="reliability-atlas-les0044-model-";A={".les0044-sentinel","cases.json","state.json","diagnoses.json"}
...
def init(x):
 r=root(x);c=load(r/'cases.json');write(r/'state.json',{'runtime':'kubernetes-storage-model-only','cases':len(c),'sha':hashlib.sha256((r/'cases.json').read_bytes()).hexdigest()});write(r/'diagnoses.json',{});print('initialize=pass cases=8')
def verify(x):
...
def diag(x,n,a=None):
 ...
 d=load(r/'diagnoses.json');d[n]=c[n];write(r/'diagnoses.json',d);print(f'diagnosis=pass case={n} stage={c[n]["stage"]} boundary={b}')
def main():
 ...
 if a.cmd=='init':init(a.root)
  ...
  for n,v in load(root(a.root)/'cases.json').items():print(f'case={n} stage={v["stage"]}')
