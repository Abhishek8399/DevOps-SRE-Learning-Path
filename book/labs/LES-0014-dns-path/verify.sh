...
readonly MODEL_SOURCE="$SCRIPT_DIRECTORY/fixtures/dns_model.py"
LAB_UID="$(id -u)"
readonly LAB_UID
...
fail() { printf 'verification_error=%s\n' "$1" >&2; return 1; }

cleanup_verifier_paths() {
  local owner resolved
  if [[ -n "$EXTERNAL_ROOT" && "$EXTERNAL_ROOT" =~ ^/tmp/reliability-atlas-LES-0014-verifier\.[[:alnum:]]{8}$ \
    ...
    if [[ "$owner" == "$LAB_UID" && "$resolved" == "$EXTERNAL_ROOT" ]]; then
      [[ -f "$EXTERNAL_ROOT/target" && ! -L "$EXTERNAL_ROOT/target" ]] && rm -- "$EXTERNAL_ROOT/target" 2>/dev/null || true
      rmdir -- "$EXTERNAL_ROOT" 2>/dev/null || true
    fi
  fi
  if [[ -n "$ORPHAN_ROOT" && "$ORPHAN_ROOT" =~ ^/tmp/reliability-atlas-LES-0014\.[[:alnum:]]{8}$ \
...
for tool in bash chmod cmp grep install ln mktemp python3 realpath rm rmdir sha256sum stat; do
  command -v "$tool" >/dev/null 2>&1 || { fail "required verifier command is missing: $tool"; exit 1; }
done
python3 -c 'from pathlib import Path; import sys; p=Path(sys.argv[1]); compile(p.read_text(encoding="utf-8"), p.name, "exec")' "$MODEL_SOURCE"
...
assert_contains() {
  ...
  grep -Fq -- "$expected" <<< "$text" || { fail "$label did not contain: $expected"; return 1; }
}
assert_absent() {
  local text="$1" forbidden="$2" label="$3"
  if grep -Fiq -- "$forbidden" <<< "$text"; then fail "$label exposed forbidden text: $forbidden"; return 1; fi
}
read_root() {
  local root owner resolved
  root="$(grep -E '^lab_root=' "$STATE_FILE")"; root="${root#lab_root=}"
  if [[ ! "$root" =~ ^/tmp/reliability-atlas-LES-0014\.[[:alnum:]]{8}$ || ! -d "$root" || -L "$root" ]]; then
    fail "registered root failed verifier boundary"; return 1
  fi
  owner="$(stat -c '%u' -- "$root")"; resolved="$(realpath -e -- "$root")"
  if [[ "$owner" != "$LAB_UID" || "$resolved" != "$root" ]]; then fail "registered root identity failed"; return 1; fi
  printf '%s' "$root"
}
...
run_case() {
  local case_name="$1" setup status baseline injected scenario observed recovered verified cleaned final view
  ...
  assert_contains "$injected" 'answer_key=not_provided' "$case_name answer isolation"
  ...
  if [[ "$case_name" == 'guided' ]]; then
    ...
    assert_contains "$scenario" 'input_name=inventory.prod' 'independent raw input'
  ...
  for view in operation resolver cache authority transport; do
  ...
  assert_contains "$verified" 'verification_scope=deterministic_model_only' "$case_name verification"
...
lab_root="$(read_root)"
printf 'unexpected\n' > "$lab_root/unexpected.file"; chmod 600 -- "$lab_root/unexpected.file"
...
chmod 700 -- "$lab_root/dns_model.py"; printf '\n# tamper\n' >> "$lab_root/dns_model.py"; chmod 500 -- "$lab_root/dns_model.py"
must_fail 'changed model status' bash "$LAB_SCRIPT" status
install -m 0500 -- "$MODEL_SOURCE" "$lab_root/dns_model.py"
assert_contains "$(bash "$LAB_SCRIPT" status)" 'state=ready' 'restored model'

EXTERNAL_ROOT="$(mktemp -d --tmpdir=/tmp 'reliability-atlas-LES-0014-verifier.XXXXXXXX')"
printf 'must-survive\n' > "$EXTERNAL_ROOT/target"; chmod 600 -- "$EXTERNAL_ROOT/target"
rm -- "$lab_root/baseline.summary"; ln -s -- "$EXTERNAL_ROOT/target" "$lab_root/baseline.summary"
...
python3 "$MODEL_SOURCE" baseline > "$lab_root/baseline.summary"; chmod 600 -- "$lab_root/baseline.summary"
...
printf '%s\n' "$descriptor" > "$STATE_FILE"; chmod 600 -- "$STATE_FILE"
bash "$LAB_SCRIPT" cleanup >/dev/null

ORPHAN_ROOT="$(mktemp -d --tmpdir=/tmp 'reliability-atlas-LES-0014.XXXXXXXX')"
...
if find "$SCRIPT_DIRECTORY" -type d -name __pycache__ -print -quit | grep -q .; then fail 'python bytecode cache residue found'; exit 1; fi
rm -- "$EXTERNAL_ROOT/target"; rmdir -- "$EXTERNAL_ROOT"; EXTERNAL_ROOT=""
trap - EXIT INT TERM
...
printf 'answer_isolation=raw-independent-inputs-no-derived-diagnosis-or-recovery\n'
