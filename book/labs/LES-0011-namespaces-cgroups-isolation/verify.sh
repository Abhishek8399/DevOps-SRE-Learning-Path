...
cleanup_verifier_paths() {
  ...
  if [[ -n "$EXTERNAL_ROOT" \
    && "$EXTERNAL_ROOT" =~ ^/tmp/reliability-atlas-LES-0011-verifier\.[[:alnum:]]{8}$ \
  ...
  if [[ -n "$ORPHAN_ROOT" \
    && "$ORPHAN_ROOT" =~ ^/tmp/reliability-atlas-LES-0011\.[[:alnum:]]{8}$ \
...
for tool in bash chmod cmp grep ln mktemp python3 realpath rm rmdir stat; do
...
read_registered_root() {
  ...
  root="${line#lab_root=}"
  if [[ ! "$root" =~ ^/tmp/reliability-atlas-LES-0011\.[[:alnum:]]{8}$ \
    || ! -d "$root" || -L "$root" ]]; then
...
host_observe="$(bash "$LAB_SCRIPT" host-observe)"
assert_contains "$host_observe" "host_observation=read-only" "host observation"
assert_contains "$host_observe" "mutation=none" "host observation"

run_case() {
  local case_name="$1" setup status baseline injected view recovered verified cleaned final
  ...
  must_fail "$case_name verify before recovery" bash "$LAB_SCRIPT" verify

  baseline="$(bash "$LAB_SCRIPT" baseline)"
  ...
  must_fail "$case_name repeated baseline" bash "$LAB_SCRIPT" baseline
  ...
  must_fail "$case_name extra argument" bash "$LAB_SCRIPT" status unexpected

  for view in identity resources events operation; do
  ...
  verified="$(bash "$LAB_SCRIPT" verify)"
  assert_contains "$verified" "record=verification" "$case_name verification"
  assert_contains "$verified" "verification_scope=deterministic-model-only" "$case_name verification"
  must_fail "$case_name repeated verification" bash "$LAB_SCRIPT" verify
...
bash "$LAB_SCRIPT" baseline >/dev/null
...
if [[ ! -f "$lab_root/unexpected.file" ]]; then
  fail "unexpected artifact was mutated after refusal"
  exit 1
fi
rm -- "$lab_root/unexpected.file"

EXTERNAL_ROOT="$(mktemp -d --tmpdir=/tmp 'reliability-atlas-LES-0011-verifier.XXXXXXXX')"
...
rm -- "$lab_root/baseline.summary"
python3 "$SCRIPT_DIRECTORY/fixtures/isolation_model.py" baseline \
  > "$lab_root/baseline.summary"
...
ORPHAN_ROOT="$(mktemp -d --tmpdir=/tmp 'reliability-atlas-LES-0011.XXXXXXXX')"
...
printf 'refusals=verify-before-recovery,repeat-baseline,second-case,invalid-input,unexpected-artifact,symlink,out-of-scope-descriptor,orphan-candidate\n'
printf 'answer_isolation=transfer-case-not-printed\n'
