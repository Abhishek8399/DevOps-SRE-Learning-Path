...
STATE="/tmp/reliability-atlas-les0070-supply-chain-$(id -u)"
SENTINEL="$STATE/.les0070"
die(){ printf 'lab=fail reason=%s\n' "$1" >&2; exit 1; }
guard(){
  [[ "$(id -u)" -ne 0 ]] || die root-refused
  command -v python3 >/dev/null 2>&1 || die python3-missing
  [[ -z "${GITHUB_TOKEN:-}${GH_TOKEN:-}${COSIGN_PASSWORD:-}${KUBECONFIG:-}${DOCKER_HOST:-}" ]] || die credential-or-external-endpoint
  [[ ! -L "$STATE" ]] || die state-symlink
  if [[ -e "$STATE" ]]; then
    [[ -d "$STATE" && -O "$STATE" && -f "$SENTINEL" && ! -L "$SENTINEL" ]] || die state-ownership
    [[ "$(<"$SENTINEL")" == "LES-0070:$(id -u)" ]] || die sentinel
  fi
}
inventory(){
  local item relative
  while IFS= read -r -d '' item; do
    relative="${item#"$STATE"/}"
    case "$relative" in .les0070|cases.json) ;; *) die "unknown-artifact:$relative" ;; esac
