...
UID_NOW="$(id -u)"
STATE="/tmp/reliability-atlas-les0059-nosql-cache-$UID_NOW"
SENTINEL="$STATE/.les0059-sentinel"
...
doctor(){
  ...
  [[ -z "${AWS_ACCESS_KEY_ID:-}${AZURE_CLIENT_SECRET:-}${GOOGLE_APPLICATION_CREDENTIALS:-}${DATABASE_URL:-}${REDIS_URL:-}${MONGODB_URI:-}${KUBECONFIG:-}" ]]||die credential
  command -v python3 >/dev/null||die python
  printf 'doctor=pass runtime=offline-nosql-cache-model\n'
}
guard(){
  [[ "$STATE" == "/tmp/reliability-atlas-les0059-nosql-cache-$UID_NOW" ]]||die unsafe-root
  [[ -d "$STATE" && ! -L "$STATE" ]]||die state
  [[ "$(stat -c %u "$STATE")" == "$UID_NOW" ]]||die owner
  [[ -f "$SENTINEL" && ! -L "$SENTINEL" && "$(<"$SENTINEL")" == "les0059:$UID_NOW" ]]||die sentinel
  while IFS= read -r -d '' entry; do
    [[ ! -L "$entry" ]]||die child-symlink
    case "$(basename -- "$entry")" in .les0059-sentinel|cases.json) ;; *) die unexpected-artifact;; esac
...
clear_unknown(){
  [[ "$STATE" == "/tmp/reliability-atlas-les0059-nosql-cache-$UID_NOW" && -d "$STATE" && ! -L "$STATE" ]]||die unsafe-clear
