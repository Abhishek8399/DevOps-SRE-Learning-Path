...
MODEL="${SCRIPT_DIR}/fixtures/slo_model.py"
SOURCE_SCENARIO="${SCRIPT_DIR}/fixtures/scenario.json"
CURRENT_UID="$(id -u)"
STATE_PARENT="/tmp"
STATE_NAME="reliability-atlas-les0032-${CURRENT_UID}"
...
validate_state() {
  [[ "${STATE_DIR}" == "/tmp/reliability-atlas-les0032-${CURRENT_UID}" ]] || { printf 'refused=true reason=state-name-invalid\n' >&2; exit 78; }
...
setup() {
  ...
  python3 -c 'import json,sys; value={"schemaVersion":1,"lessonId":"LES-0032","uid":int(sys.argv[1]),"statePath":sys.argv[2],"caseId":"slo-control-loop-v1"}; handle=open(sys.argv[3],"w",encoding="utf-8"); json.dump(value,handle,indent=2); handle.write("\n"); handle.close()' "${CURRENT_UID}" "${STATE_DIR}" "${candidate}/manifest.json"
...
status() {
  ...
  printf 'state=ready path=%s case=slo-control-loop-v1 results=%s runtime=deterministic-model-only\n' "${STATE_DIR}" "${result_count}"
}
...
cleanup() {
  ...
  [[ "${resolved}" == "/tmp/reliability-atlas-les0032-${CURRENT_UID}" ]] || { printf 'refused=true reason=cleanup-realpath-invalid\n' >&2; exit 78; }
...
main() {
  ...
  case "${action}" in
    ...
    cleanup) cleanup ;;
    reset) cleanup; setup ;;
    *) printf 'usage: bash lab.sh {doctor|check|setup|status|run <case>|cleanup|reset}\n' >&2; exit 64 ;;
  esac
}
