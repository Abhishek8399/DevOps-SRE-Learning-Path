...
UID_NOW="$(id -u)"
STATE="/tmp/reliability-atlas-les0065-specialized-data-$UID_NOW"
SENTINEL="$STATE/.les0065-sentinel"
...
doctor(){
  ...
  [[ -z "${CASSANDRA_PASSWORD:-}${CQLSH_HOST:-}${QDRANT_API_KEY:-}${QDRANT_URL:-}${OPENMETADATA_JWT_TOKEN:-}${OPENMETADATA_HOST_PORT:-}${KUBECONFIG:-}" ]]||die credential
  command -v python3 >/dev/null||die python
  printf 'doctor=pass runtime=offline-specialized-data-model\n'
}
guard(){
  [[ "$STATE" == "/tmp/reliability-atlas-les0065-specialized-data-$UID_NOW" ]]||die unsafe-root
  [[ -d "$STATE" && ! -L "$STATE" ]]||die state
  [[ "$(stat -c %u "$STATE")" == "$UID_NOW" ]]||die owner
  [[ -f "$SENTINEL" && ! -L "$SENTINEL" && "$(<"$SENTINEL")" == "les0065:$UID_NOW" ]]||die sentinel
  while IFS= read -r -d '' entry; do
    [[ ! -L "$entry" ]]||die child-symlink
    case "$(basename -- "$entry")" in .les0065-sentinel|cases.json) ;; *) die unexpected-artifact;; esac
...
clear_unknown(){
  [[ "$STATE" == "/tmp/reliability-atlas-les0065-specialized-data-$UID_NOW" && -d "$STATE" && ! -L "$STATE" ]]||die unsafe-clear
