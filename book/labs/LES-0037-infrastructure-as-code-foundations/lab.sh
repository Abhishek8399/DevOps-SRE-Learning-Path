...
validate_state() {
  [[ "${STATE_DIR}" == "/tmp/reliability-atlas-les0037-${CURRENT_UID}" ]] || { printf 'refused=true reason=state-path-invalid\n' >&2; exit 78; }
...
setup() {
  ...
  local candidate
  candidate="$(mktemp -d "${STATE_PARENT}/reliability-atlas-les0037-${CURRENT_UID}.candidate.XXXXXX")"
  cleanup_candidate() {
    if [[ -n "${candidate:-}" && -d "${candidate}" && ! -L "${candidate}" ]]; then
      local resolved
      resolved="$(readlink -f -- "${candidate}")"
      if [[ "${resolved}" == "/tmp/reliability-atlas-les0037-${CURRENT_UID}.candidate."* && "$(stat -c '%u' -- "${resolved}")" == "${CURRENT_UID}" ]]; then
        rm -rf -- "${resolved}"
      fi
    fi
  }
  ...
  python3 -c 'import json,sys; value={"schemaVersion":1,"lessonId":"LES-0037","uid":int(sys.argv[1]),"statePath":sys.argv[2],"caseId":"iac-change-system-v1"}; handle=open(sys.argv[3],"w",encoding="utf-8"); json.dump(value,handle,indent=2); handle.write("\n"); handle.close()' "${CURRENT_UID}" "${STATE_DIR}" "${candidate}/manifest.json"
...
run_case() {
  local case_name="${1:-}"
  [[ -n "${case_name}" ]] || { printf 'usage: bash lab.sh run <graph|plan|drift|policy|partial|converge|sensitive>\n' >&2; exit 64; }
  require_tools
  validate_inputs
...
cleanup() {
  ...
  [[ "${resolved}" == "/tmp/reliability-atlas-les0037-${CURRENT_UID}" ]] || { printf 'refused=true reason=cleanup-path-invalid\n' >&2; exit 78; }
