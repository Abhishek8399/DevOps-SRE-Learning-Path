...
STATE="/tmp/reliability-atlas-les0069-ai-security-$(id -u)"
SENTINEL="$STATE/.les0069"
...
guard(){
  ...
  if [[ -e "$STATE" ]]; then
    ...
    [[ "$(<"$SENTINEL")" == "LES-0069:$(id -u)" ]] || die sentinel
  fi
}
inventory(){
  local item relative
  while IFS= read -r -d '' item; do
    relative="${item#"$STATE"/}"
    case "$relative" in .les0069|cases.json) ;; *) die "unknown-artifact:$relative" ;; esac
