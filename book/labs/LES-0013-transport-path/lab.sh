...
readonly LESSON_ID="LES-0013"
readonly LAB_VERSION="1"
SCRIPT_DIRECTORY="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
readonly SCRIPT_DIRECTORY
readonly FIXTURE_SOURCE="$SCRIPT_DIRECTORY/fixtures/transport_model.py"
LAB_UID="$(id -u)"
readonly LAB_UID
readonly LAB_PREFIX="reliability-atlas-LES-0013."
readonly STATE_FILE="/tmp/reliability-atlas-LES-0013-$LAB_UID.state"
readonly SENTINEL_NAME=".les-0013-sentinel"
readonly MANIFEST_NAME="artifact-manifest.tsv"
readonly MODEL_NAME="transport_model.py"
...
readonly EXPECTED_MANIFEST=$'artifact\ttype\tcreated_by\trequired\n.les-0013-sentinel\tregular-file\tsetup\tyes\nartifact-manifest.tsv\tregular-file\tsetup\tyes\ntransport_model.py\tregular-file\tsetup\tyes\nbaseline.summary\tregular-file\trun-baseline\tno\nactive-case.state\tregular-file\tinject\tno\nrecovery.summary\tregular-file\trecover\tno\nverification.summary\tregular-file\tverify-operation\tno'
readonly BASELINE_KEYS="record case operation operation_success error connect_latency_ms new_connections_per_second established_reuse_percent active_connections syn_sent_connections syn_recv_connections time_wait_connections close_wait_connections listen_queue_current listen_queue_limit listen_overflows_total ephemeral_ports_used ephemeral_ports_eligible process_fds_used process_fds_limit socket_memory_bytes conntrack_entries conntrack_limit conntrack_insert_failures_total"
readonly OPERATION_KEYS="record case view operation operation_success error phase connect_latency_ms established_reuse_success"
readonly ENDPOINT_KEYS="record case view protocol address_family client_state server_listener active_connections syn_sent_connections syn_recv_connections time_wait_connections close_wait_connections"
readonly QUEUE_KEYS="record case view listen_queue_current listen_queue_limit listen_overflows_total accept_rate_per_second new_connections_per_second"
readonly RESOURCE_KEYS="record case view ephemeral_ports_used ephemeral_ports_eligible process_fds_used process_fds_limit socket_memory_bytes"
readonly STATEFUL_KEYS="record case view state_scope conntrack_entries conntrack_limit conntrack_insert_failures_total nat_mappings_active"
readonly RECOVERY_KEYS="record case action operation_success error listen_queue_current conntrack_entries"
readonly VERIFICATION_KEYS="record case operation operation_success fresh_connect_success established_reuse_success correctness_check verification_scope"
...
list_owned_orphan_candidates() {
  ...
  for candidate in "${entries[@]}"; do
    if [[ ! "$candidate" =~ ^/tmp/reliability-atlas-LES-0013\.[[:alnum:]]{8}$ \
...
validate_root_path() {
  ...
  if [[ ! "$candidate" =~ ^/tmp/reliability-atlas-LES-0013\.[[:alnum:]]{8}$ ]]; then
...
compare_recorded_output() {
  ...
  if ! cmp -s -- "$path" <(printf '%s\n' "$expected"); then
    fail "$label content changed; use guarded reset if identity remains valid"
    return 1
  fi
}
...
rollback_setup() {
  ...
  if [[ -n "$ROLLBACK_ROOT" \
    && "$ROLLBACK_ROOT" =~ ^/tmp/reliability-atlas-LES-0013\.[[:alnum:]]{8}$ \
...
command_check() {
  ...
  printf 'network=none\n'
  printf 'execution=deterministic_transport_model\n'
  if state_file_present; then
    load_state
...
command_status() {
  ...
  printf 'execution=deterministic_transport_model\n'
  printf 'host_network_mutation=none\n'
}

command_run() {
  local run_name="$1" output path
  ...
  if path_present "$path"; then
    fail "baseline was already recorded; use guarded cleanup and setup for a fresh attempt"
    return 1
  fi
  if [[ -n "$ACTIVE_CASE" ]]; then
...
command_inject() {
  ...
  if [[ -n "$ACTIVE_CASE" ]]; then
    fail "an incident case is already active; clean and set up for another case"
    return 1
  fi
  ...
  printf 'case=%s\n' "$case_name"
  printf 'failure_scope=deterministic_model_only\n'
...
command_observe() {
  ...
  case "$view" in
    ...
    endpoints) keys="$ENDPOINT_KEYS" ;;
    queues) keys="$QUEUE_KEYS" ;;
    resources) keys="$RESOURCE_KEYS" ;;
    stateful-path) keys="$STATEFUL_KEYS" ;;
    *)
      fail "view must be operation, endpoints, queues, resources, or stateful-path"
      return 1
      ;;
  esac
  require_environment
  ...
  if path_present "$LAB_ROOT/$RECOVERY_NAME"; then
    fail "the virtual case is already recovered; verify-operation or clean"
    return 1
  fi
...
command_recover() {
  ...
  if path_present "$LAB_ROOT/$RECOVERY_NAME"; then
    fail "recovery was already recorded; verify-operation or clean"
    return 1
  fi
...
command_verify_operation() {
  ...
  if path_present "$LAB_ROOT/$VERIFICATION_NAME"; then
    fail "verification was already recorded; inspect status or clean"
    return 1
  fi
...
usage() {
  printf '%s\n' \
    ...
    "       bash lab.sh observe operation|endpoints|queues|resources|stateful-path" \
    "       bash lab.sh recover" \
...
main() {
  local command="${1:-}"
  case "$command" in
    check|setup|status|recover|verify-operation|cleanup)
      if [[ "$#" -ne 1 ]]; then
