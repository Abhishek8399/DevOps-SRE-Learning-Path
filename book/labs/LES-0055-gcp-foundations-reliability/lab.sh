...
ROOT="/tmp/reliability-atlas-les0055-model-$UID_NOW"
...
doctor(){
  ...
  [[ -z "${GOOGLE_APPLICATION_CREDENTIALS:-}${CLOUDSDK_CONFIG:-}${GOOGLE_CLOUD_PROJECT:-}${GCLOUD_PROJECT:-}" ]]||die credential
  ...
  printf 'doctor=pass runtime=gcp-readiness-model-only\n'
}
need(){ doctor>/dev/null; [[ -d "$ROOT"&&! -L "$ROOT" ]]||die state; python3 "$MODEL" status "$ROOT">/dev/null; }
setup(){ doctor; [[ ! -e "$ROOT"&&! -L "$ROOT" ]]||die exists; mkdir -m 0700 "$ROOT"; cp -- "$FIXTURE" "$ROOT/cases.json"; printf 'les0055:%s\n' "$UID_NOW">"$ROOT/.les0055-sentinel"; chmod 0600 "$ROOT/cases.json" "$ROOT/.les0055-sentinel"; python3 "$MODEL" init "$ROOT"; }
...
clear_unknown(){ [[ "$ROOT" == "/tmp/reliability-atlas-les0055-model-$UID_NOW"&&-d "$ROOT"&&! -L "$ROOT" ]]||die unsafe-clear; [[ "$(stat -c %u "$ROOT")" == "$UID_NOW" ]]||die unsafe-owner; [[ -f "$ROOT/unexpected"&&! -L "$ROOT/unexpected" ]]||die unknown-shape; rm -f -- "$ROOT/unexpected"; need; }
case "${1:-}" in
