...
readonly LESSON_ID='LES-0022'
readonly STATE_VERSION='1'
readonly STATE_PARENT='/tmp'
readonly ROOT_PREFIX='reliability-atlas-LES-0022.'
...
LAB_UID=$EUID
readonly LAB_UID
readonly STATE_FILE="${STATE_PARENT}/reliability-atlas-LES-0022-${LAB_UID}.state"
...
usage:
  ...
  bash lab.sh observe inputs|dependencies|context|cache|artifact|supplychain
  bash lab.sh recover
...
validate_control_environment() {
  case ${LAB_DRY_RUN:-0} in
    0|1) ;;
    *) die 'LAB_DRY_RUN-must-be-0-or-1' 64 ;;
  esac
  case ${LAB_TEST_STOP_AFTER_CLEANUP_MARKER:-0} in
    0|1) ;;
    *) die 'LAB_TEST_STOP_AFTER_CLEANUP_MARKER-must-be-0-or-1' 64 ;;
  esac
  case ${LES0022_VERIFIER_MODE:-0} in
    0|1) ;;
    *) die 'LES0022_VERIFIER_MODE-must-be-0-or-1' 64 ;;
  esac
  if [[ ${LAB_TEST_STOP_AFTER_CLEANUP_MARKER:-0} == 1 && ${LES0022_VERIFIER_MODE:-0} != 1 ]]; then
    die 'cleanup-interruption-hook-is-verifier-only' 64
  fi
}
...
load_state_descriptor() {
  ...
  [[ $LAB_ROOT =~ ^/tmp/reliability-atlas-LES-0022\.[A-Za-z0-9]{8}$ ]] \
...
validate_normal_state() {
  ...
  validate_root_identity
  validate_existing_children
  ! path_present "${LAB_ROOT}/cleanup.marker" || die 'cleanup-is-in-progress-use-cleanup-to-resume' 73
  validate_sentinel
  validate_regular_file "${LAB_ROOT}/build_model.py" 500
  validate_regular_file "${LAB_ROOT}/.lock" 600
  cmp -s -- "$MODEL_SOURCE" "${LAB_ROOT}/build_model.py" || die 'installed-model-differs-from-reviewed-source' 73
}
...
pending_setup_cleanup() {
  ...
  if [[ -n $PENDING_ROOT ]]; then
    ...
    if ((registered == 0)) && [[ $PENDING_ROOT =~ ^/tmp/reliability-atlas-LES-0022\.[A-Za-z0-9]{8}$ && -d $PENDING_ROOT && ! -L $PENDING_ROOT ]]; then
      if [[ $(stat -c '%u' -- "$PENDING_ROOT" 2>/dev/null || printf invalid) == "$LAB_UID" ]]; then
        for child in record.tmp verification.record recovery.record case.record baseline.record cleanup.marker .lock build_model.py .sentinel; do
...
command_check() {
  ...
  if ! path_present "$LAB_ROOT"; then
    die 'descriptor-points-to-absent-root-run-cleanup-to-finalize' 73
  fi
  if path_present "${LAB_ROOT}/cleanup.marker"; then
    validate_cleanup_state
...
command_setup() {
  ...
  if [[ ${LAB_DRY_RUN:-0} == 1 ]]; then
    require_absent_state
    printf 'dry_run=true\naction=setup\nwould_create=one-private-random-root-and-one-descriptor\nnetwork=%s\nstate=absent\n' "$NETWORK_POLICY"
    return 0
  fi
  if path_present "$STATE_FILE"; then
    validate_normal_state
  ...
  install -m 0500 -- "$MODEL_SOURCE" "${root}/build_model.py"
  : >"${root}/.lock"
  chmod 600 -- "${root}/.lock"

  descriptor_candidate=$(mktemp --tmpdir="$STATE_PARENT" "reliability-atlas-LES-0022-${LAB_UID}.candidate.XXXXXXXX") \
...
command_run() {
  ...
  output=$(PYTHONDONTWRITEBYTECODE=1 python3 "${LAB_ROOT}/build_model.py" baseline)
  require_field "$output" 'record=baseline'
  require_field "$output" 'byte_identical=true'
...
command_inject() {
  ...
  output=$(PYTHONDONTWRITEBYTECODE=1 python3 "${LAB_ROOT}/build_model.py" case --name "$case_name")
...
command_scenario() {
  ...
  output=$(PYTHONDONTWRITEBYTECODE=1 python3 "${LAB_ROOT}/build_model.py" scenario)
...
command_observe() {
  ...
  case $view in
    inputs|dependencies|context|cache|artifact|supplychain) ;;
  ...
  output=$(PYTHONDONTWRITEBYTECODE=1 python3 "${LAB_ROOT}/build_model.py" observe --case "$case_name" --view "$view")
...
command_recover() {
  ...
  output=$(PYTHONDONTWRITEBYTECODE=1 python3 "${LAB_ROOT}/build_model.py" recover --case "$case_name")
...
command_verify_operation() {
  ...
  output=$(PYTHONDONTWRITEBYTECODE=1 python3 "${LAB_ROOT}/build_model.py" verify --case "$case_name")
  require_field "$output" 'record=verification'
  require_field "$output" 'operation_success=true'
  require_field "$output" 'byte_identical=true'
  require_field "$output" 'lock_integrity=valid'
  require_field "$output" 'context_allowlist=valid'
...
remove_if_present() {
  local name=$1 expected_mode=$2 path
  path="${LAB_ROOT}/${name}"
...
command_cleanup() {
  local root_before marker resuming=0
  validate_environment
  if ! path_present "$STATE_FILE"; then
    require_absent_state
  ...
  if ! path_present "${LAB_ROOT}/cleanup.marker"; then
    acquire_lock
    marker="${LAB_ROOT}/cleanup.marker"
    printf 'lesson=%s\nuid=%s\nroot=%s\n' "$LESSON_ID" "$LAB_UID" "$LAB_ROOT" >"$marker"
    chmod 600 -- "$marker"
    validate_cleanup_marker
    release_lock
    if [[ ${LAB_TEST_STOP_AFTER_CLEANUP_MARKER:-0} == 1 ]]; then
      die 'simulated-interruption-after-cleanup-marker' 85
    fi
  fi
  validate_cleanup_state

  remove_if_present verification.record 600
  remove_if_present recovery.record 600
  remove_if_present case.record 600
  remove_if_present baseline.record 600
  remove_if_present record.tmp 600
  remove_if_present build_model.py 500
  remove_if_present .sentinel 400
  remove_if_present .lock 600
  remove_if_present cleanup.marker 600
