...
STATE="/tmp/reliability-atlas-les0067-aiops-$(id -u)"
SENTINEL="$STATE/.les0067"
...
guard(){
  ...
  if [[ -e "$STATE" ]]; then
    ...
    [[ "$(<"$SENTINEL")" == "LES-0067:$(id -u)" ]] || die sentinel
  fi
}
inventory(){
  local item relative
  while IFS= read -r -d '' item; do
    relative="${item#"$STATE"/}"
    case "$relative" in .les0067|cases.json) ;; *) die "unknown-artifact:$relative" ;; esac
