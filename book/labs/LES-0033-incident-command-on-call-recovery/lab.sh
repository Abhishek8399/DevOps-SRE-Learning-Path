...
CURRENT_UID="$(id -u)"
STATE_PARENT="/tmp"
STATE_NAME="reliability-atlas-les0033-${CURRENT_UID}"
...
validate_state() {
  [[ "${STATE_DIR}" == "/tmp/reliability-atlas-les0033-${CURRENT_UID}" ]] || { printf 'refused=true reason=state-name-invalid\n' >&2; exit 78; }
...
setup() {
  ...
  python3 -c 'import json,sys; value={"schemaVersion":1,"lessonId":"LES-0033","uid":int(sys.argv[1]),"statePath":sys.argv[2],"caseId":"incident-command-v1"}; handle=open(sys.argv[3],"w",encoding="utf-8"); json.dump(value,handle,indent=2); handle.write("\n"); handle.close()' "${CURRENT_UID}" "${STATE_DIR}" "${candidate}/manifest.json"
...
status() {
  ...
  printf 'state=ready path=%s case=incident-command-v1 results=%s runtime=deterministic-model-only\n' "${STATE_DIR}" "${result_count}"
}
...
cleanup() {
  ...
  [[ "${resolved}" == "/tmp/reliability-atlas-les0033-${CURRENT_UID}" ]] || { printf 'refused=true reason=cleanup-realpath-invalid\n' >&2; exit 78; }
