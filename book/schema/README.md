...
`LES-0006` is the first schema-backed lesson. The current canonical worktree contains twenty-one structured `substantive-draft` lessons (`LES-0006` through `LES-0026`) across Volumes 00 through 04, 63 assessments—forty-two complete-answer records and twenty-one answer-isolated independent transfers—and 172 references. Together with the five reserved typed lessons, the reader target exposes twenty-six routed identities. Use the next unused opaque IDs for new work: `LES-0027`, `ASM-0064`, and `REF-0173` at this checkpoint. The LES-0026 promotion remains uncommitted until final integrated gates and remote parity are recorded. An ID is permanent once published and is never recycled. Schema validity and publication do not constitute formal chapter acceptance, provider execution, or learner mastery.
