...
`LES-0002` is the first published legacy body migrated into the schema contract; `LES-0006` was the first newly authored schema-backed lesson. Canonical checkpoint `f3b9f1b` contains twenty-two structured `substantive-draft` lessons, 66 assessments—forty-four complete-answer records and twenty-two answer-isolated independent transfers—and 175 references. Together with four remaining typed bodies, the reader still exposes exactly twenty-six routed identities. `LES-0027` through `LES-0092` and support IDs `ASM-0064` through `ASM-0261` plus `REF-0173` through `REF-1199` belong to staged authored packages; do not reuse them. The first free support IDs after this checkpoint are `ASM-0265` and `REF-1203`. A published legacy identity remains in the permanent map after migration, and its ID, route, slug, aliases, curriculum ownership, search identity, and learning-state key remain compatibility contracts. An ID is permanent once published and is never recycled. Schema validity and publication do not constitute formal chapter acceptance, provider execution, or learner mastery.
