...
`LES-0001`, `LES-0002`, `LES-0003`, and `LES-0005` are published legacy bodies migrated into the schema contract; `LES-0006` was the first newly authored schema-backed lesson. The canonical corpus contains twenty-five structured `substantive-draft` lessons, 75 assessments—fifty complete-answer records and twenty-five answer-isolated independent transfers—and 193 references. Together with one remaining typed body, the reader still exposes exactly twenty-six routed identities. `LES-0027` through `LES-0092` and support IDs `ASM-0064` through `ASM-0261` plus `REF-0173` through `REF-1199` belong to staged authored packages; do not reuse them. The next free support IDs are `ASM-0274` and `REF-1221`. A published legacy identity remains in the permanent map after migration, and its ID, route, slug, aliases, curriculum ownership, search identity, and learning-state key remain compatibility contracts. An ID is permanent once published and is never recycled. Schema validity and publication do not constitute formal chapter acceptance, provider execution, or learner mastery.
