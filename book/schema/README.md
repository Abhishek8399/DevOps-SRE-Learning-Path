...
`LES-0006` is the first schema-backed lesson. Canonical feature `f2e3e23` contains twenty-one structured `substantive-draft` lessons (`LES-0006` through `LES-0026`) across Volumes 00 through 04, 63 assessments—forty-two complete-answer records and twenty-one answer-isolated independent transfers—and 172 references. Together with the five reserved typed lessons, the reader exposes twenty-six routed identities. Use the next unused opaque IDs for new work: `LES-0027`, `ASM-0064`, and `REF-0173` at this checkpoint. LES-0026 is pushed with exact remote parity. An ID is permanent once published and is never recycled. Schema validity and publication do not constitute formal chapter acceptance, provider execution, or learner mastery.
