...
`LES-0006` is the first schema-backed lesson. The validated authored corpus contains eighteen structured lessons (`LES-0006` through `LES-0023`) across Volumes 00 through 03, fifty-four assessments, and 144 references. Together with the five reserved typed lessons, the reader exposes twenty-three routed identities. Content validation passes, and the 39-case schema suite reports 38 passes, zero failures, and one Windows `EPERM` symlink skip. Use the next unused opaque ID for new work (`LES-0024` at this checkpoint); an ID is permanent once published and is never recycled.
