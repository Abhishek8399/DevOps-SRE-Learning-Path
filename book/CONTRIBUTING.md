...
Follow [`schema/README.md`](schema/README.md), inspect canonical and staged registries before allocating IDs, and run both contract commands before review. The canonical corpus contains twenty-three structured `substantive-draft` lessons, 69 assessments, and 181 references; `LES-0002` and `LES-0003` are exact-identity migrations and three typed bodies remain. Staged `LES-0027` through `LES-0092` reserve `ASM-0064` through `ASM-0261` and `REF-0173` through `REF-1199`; the next free support IDs are `ASM-0268` and `REF-1209`. Every chapter still requires formal review and independently reviewed learner evidence, and none establishes mastery. Choose editorial work from [`MASTER_PLAN.md`](../MASTER_PLAN.md) rather than numeric sequence. The permanent legacy map remains after migration and prevents any record from stealing a published ID, route, slug, alias, curriculum mapping, search identity, or learning-state key.
