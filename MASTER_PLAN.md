...
| `PLAN-ARC-004` | P1 | Structured Markdown content schema | `PLAN-ARC-002`, `PLAN-ARC-003` | `PARTIAL` | New content lives under `book/volumes/...`; strict JSON front matter covers metadata; diagrams, command cards, incidents, answers, lab links, assessments, references, and legacy identities have validated structures | 34 focused tests plus live repository validation; first production record migration remains |
| `PLAN-ARC-005` | P1 | Typed-to-structured-Markdown migration | `PLAN-ARC-004` | `PLANNED` | Existing five lessons migrate without URL, ID, text, diagram, answer, lab, search, or device-state regression | Before/after route snapshots; content parity and state-migration tests |
...
| `PLAN-ARC-007` | P1 | Reference registry | `PLAN-GOV-006`, `PLAN-ARC-004` | `PARTIAL` | Primary references record title, canonical HTTPS URL, version/date, lesson IDs, review window, relevance, and paraphrase policy | Strict schema and relationship tests exist; live reference population and freshness review remain |
...
| `PLAN-QUA-002` | P0 | Content/schema validation | `PLAN-ARC-004` | `PARTIAL` | Schemas fail closed; IDs, legacy reservations, prerequisites, metadata, non-empty sections, answers, references, command assertions, paths, ownership, backlinks, and cycles validate | 34 dependency-free unit and disposable-repository tests; CI wiring and production record corpus remain |
