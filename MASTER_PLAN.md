...
| `PLAN-GOV-004` | P0 | Contribution and review workflow | `PLAN-GOV-003` | `PARTIAL` | A future human or AI can add content without chat history; safety and definition-of-done checks are explicit | `LES-0006` and `LES-0007` exercised the structured authoring, validation, independent-review, and correction path; fresh-contributor dry-run remains |
...
| `PLAN-GOV-006` | P1 | Reference freshness policy | `PLAN-GOV-003` | `PARTIAL` | Version-sensitive claims identify primary sources, tested versions, review dates, and expiration/recheck rules | Sixteen records across `LES-0006` and `LES-0007` include versions or review context and review windows; repository-wide stale-reference report remains |
...
| `PLAN-ARC-004` | P1 | Structured Markdown content schema | `PLAN-ARC-002`, `PLAN-ARC-003` | `COMMITTED` | New content lives under `book/volumes/...`; strict JSON front matter covers metadata; diagrams, command cards, incidents, answers, lab links, assessments, references, and legacy identities have validated structures | 39 focused cases (38 pass and one Windows EPERM symlink case skips), two live structured lessons, safe renderer, exact virtual source registry, build, route, and answer-isolation validation |
| `PLAN-ARC-005` | P1 | Typed-to-structured-Markdown migration | `PLAN-ARC-004` | `PARTIAL` | Existing five lessons migrate without URL, ID, text, diagram, answer, lab, search, or device-state regression | Seven-entry mixed catalog and additive six-entry-v1-to-seven-entry state migration are tested; migration of the five legacy lesson bodies remains |
...
| `PLAN-ARC-007` | P1 | Reference registry | `PLAN-GOV-006`, `PLAN-ARC-004` | `PARTIAL` | Primary references record title, canonical HTTPS URL, version/date, lesson IDs, review window, relevance, and paraphrase policy | Strict relationships plus sixteen live references across `LES-0006` and `LES-0007` pass; full-curriculum population and freshness report remain |
| `PLAN-ARC-008` | P2 | Content search index | `PLAN-ARC-002`, `PLAN-ARC-003` | `PARTIAL` | Local search finds terms, symptoms, commands, tools, and lesson IDs without an external service | Seven-document cross-volume production catalog and deterministic golden queries pass; filters, large-corpus scaling, and disconnected-browser proof remain |

## Website capabilities

| ID | Pri | Deliverable | Depends on | Status | Acceptance criteria | Verification |
|---|---:|---|---|---|---|---|
| `PLAN-WEB-001` | P0 | Loopback-only local launcher | None | `COMMITTED` | Windows launcher installs locked dependencies when needed, binds locally, opens the reader, and documents stop behavior | Corrected Vite `--hostname` launch is proven on loopback in a fresh development process; fresh-clone Windows/WSL launch smoke remains |
| `PLAN-WEB-002` | P0 | Routed book reader | `PLAN-ARC-001` | `PARTIAL` | Separate library, volume, lesson, and practice routes; breadcrumbs; valid static params; invalid lessons return 404 | Fourteen declared routes and invalid lesson 404s pass across Volume 00 and Volume 01; full internal-link and manual navigation review remain |
| `PLAN-WEB-003` | P1 | Persistent book navigation | `PLAN-WEB-002` | `PARTIAL` | Desktop sidebar and mobile contents expose current lessons and clearly label planned volumes | Volume 00/01 catalog, breadcrumbs, indexes, and pagination are covered by reader tests; keyboard/mobile/manual navigation remains |
...
| `PLAN-WEB-005` | P1 | Find and resume | `PLAN-ARC-008`, `PLAN-WEB-003` | `PARTIAL` | Search, stable deep links, recent location, and bookmarks work locally; no convenience state becomes mastery evidence | Seven IDs and additive prior-state migration pass without creating mastery data; browser restart, cross-tab, and disconnected interaction tests remain |
...
|---|---:|---|---|---|---|---|
| `PLAN-CUR-000` | P0 | Volume 00: start safely | `PLAN-GOV-003`, `PLAN-LAB-001` | `PARTIAL` | Ubuntu setup, shell behavior, WSL boundaries, Git workflow, command risk, secrets, evidence, FRAME incidents, OPERATES reviews, SCALE decisions, rollback, and cleanup | `LES-0007` now provides routed systems-thinking foundations, three assessments, eight references, and a queue lab with a passing Ubuntu verifier; safe setup, shell/WSL/Git/secrets coverage, formal review, and learner evidence remain |
...
| `PLAN-INT-006` | P0 | Rubrics and mastery separation | `PLAN-GOV-005`, `PLAN-INT-001` | `PARTIAL` | Scores reasoning, evidence, safety, trade-offs, communication, and independence; website score never auto-promotes competency | Six assessment rubrics reconcile and both independent transfers remain answer-isolated; ledger authorization and complete evidence-lineage audits remain |
...
| `PLAN-QUA-002` | P0 | Content/schema validation | `PLAN-ARC-004` | `PARTIAL` | Schemas fail closed; IDs, legacy reservations, prerequisites, metadata, non-empty sections, answers, references, command assertions, paths, ownership, backlinks, and cycles validate | 39 dependency-free cases exercise two lessons, six assessments, and sixteen references; 38 pass and one Windows EPERM symlink case skips; CI wiring remains |
...
| `PLAN-QUA-004` | P1 | Route, link, and 404 tests | `PLAN-WEB-002`, `PLAN-ARC-004` | `PARTIAL` | All declared routes return expected status; links and anchors resolve; invalid IDs return 404 | Fourteen-route/404 smoke passes across Volume 00 and Volume 01; automated internal crawl and browser navigation remain |
...
| `PLAN-AUD-005` | P0 | Mastery-integrity audit | `PLAN-GOV-005`, `PLAN-INT-006` | `PARTIAL` | Reading, button clicks, localStorage, model answers, and mentor-generated output cannot be mistaken for learner evidence | Twenty reader tests include additive state migration, explicit non-mastery assertions, and answer isolation for both structured transfers; full evidence-lineage review remains |
...
1. Continue the dependency-ordered Volume 00/Linux structured batch after `LES-0007`, with deep explanations, command decoders, complete answers, bounded Ubuntu labs, and primary references.
...
3. Add search filters, internal crawling, and browser-level persistence, keyboard, clipboard, night, mobile, print, and visual checks across both routed volumes.
4. Restore Docker integration in Ubuntu, then run the full-envelope v2 lifecycle, one-field counterfeit refusals, check/reset paths, and exact legacy-v1 retirement.
5. Complete remaining host-lab failure matrices, ShellCheck, accessibility/privacy/performance checks, dependency/license review, CI wiring, and fresh-clone reproducibility before `PLAN-AUD-001`.
