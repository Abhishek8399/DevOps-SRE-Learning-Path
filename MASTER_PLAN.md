...
| `PLAN-GOV-004` | P0 | Contribution and review workflow | `PLAN-GOV-003` | `PARTIAL` | A future human or AI can add content without chat history; safety and definition-of-done checks are explicit | `LES-0006` through `LES-0008` exercise the validated structured path; a fresh-contributor dry-run remains |
...
| `PLAN-GOV-006` | P1 | Reference freshness policy | `PLAN-GOV-003` | `PARTIAL` | Version-sensitive claims identify primary sources, tested versions, review dates, and expiration/recheck rules | All 24 records across `LES-0006` through `LES-0008` pass root relationship validation; a repository-wide stale-reference report remains |
...
| `PLAN-ARC-004` | P1 | Structured Markdown content schema | `PLAN-ARC-002`, `PLAN-ARC-003` | `COMMITTED` | New content lives under `book/volumes/...`; strict JSON front matter covers metadata; diagrams, command cards, incidents, answers, lab links, assessments, references, and legacy identities have validated structures | The 39-case suite covers three lessons, nine assessments, and 24 references with 38 passes and one Windows `EPERM` symlink skip |
| `PLAN-ARC-005` | P1 | Typed-to-structured-Markdown migration | `PLAN-ARC-004` | `PARTIAL` | Existing five lessons migrate without URL, ID, text, diagram, answer, lab, search, or device-state regression | The eight-entry mixed catalog and additive seven-entry-v1-to-eight-entry state migration pass 21 reader tests; migration of the five legacy lesson bodies remains |
...
| `PLAN-ARC-007` | P1 | Reference registry | `PLAN-GOV-006`, `PLAN-ARC-004` | `PARTIAL` | Primary references record title, canonical HTTPS URL, version/date, lesson IDs, review window, relevance, and paraphrase policy | All 24 references across three structured lessons pass root relationship validation; full-curriculum population and freshness reporting remain |
| `PLAN-ARC-008` | P2 | Content search index | `PLAN-ARC-002`, `PLAN-ARC-003` | `PARTIAL` | Local search finds terms, symptoms, commands, tools, and lesson IDs without an external service | Canonical `LES-0001` through `LES-0008` search passes the reader suite and route smoke; filters, large-corpus scaling, and disconnected-browser proof remain |
...
| `PLAN-WEB-002` | P0 | Routed book reader | `PLAN-ARC-001` | `PARTIAL` | Separate library, volume, lesson, and practice routes; breadcrumbs; valid static params; invalid lessons return 404 | Fifteen declared routes pass with one `h1`, two invalid routes return 404, 15 discovered internal links are non-error, and the Reliability Atlas homepage journey preserves real versus planned route boundaries; manual navigation remains |
| `PLAN-WEB-003` | P1 | Persistent book navigation | `PLAN-WEB-002` | `PARTIAL` | Desktop sidebar and mobile contents expose current lessons and clearly label planned volumes | Volume navigation, pagination, and advisory prerequisites pass automated tests and route payload checks; keyboard/mobile/manual verification remains blocked by browser availability |
...
| `PLAN-WEB-005` | P1 | Find and resume | `PLAN-ARC-008`, `PLAN-WEB-003` | `PARTIAL` | Search, stable deep links, recent location, and bookmarks work locally; no convenience state becomes mastery evidence | Eight allowlisted IDs and additive seven-to-eight state migration pass 21 reader tests without mastery data; browser restart, cross-tab, and disconnected interaction evidence remain |
...
|---|---:|---|---|---|---|---|
| `PLAN-CUR-000` | P0 | Volume 00: start safely | `PLAN-GOV-003`, `PLAN-LAB-001` | `PARTIAL` | Ubuntu setup, shell behavior, WSL boundaries, Git workflow, command risk, secrets, evidence, FRAME incidents, OPERATES reviews, SCALE decisions, rollback, and cleanup | `LES-0007` and `LES-0008` pass their current project gates and mentor-operated Ubuntu verifiers; both remain `substantive-draft`, while safe setup, shell/WSL/Git/secrets, formal review, and learner evidence remain |
...
| ID | Pri | Deliverable | Depends on | Status | Acceptance criteria | Verification |
|---|---:|---|---|---|---|---|
| `PLAN-INT-001` | P0 | Question and answer schema | `PLAN-GOV-003` | `PARTIAL` | Stable opaque ID, lesson ownership, type, difficulty, prompt, direct answer, foundation, reasoning, senior answer, weak answer, evidence, answered follow-ups, rubric, and non-mastery boundary | All nine records across three lessons pass schema/reader verification; `ASM-0009` remains answer-isolated, while broad bank migration remains |
| `PLAN-INT-002` | P0 | Linux question bank | `PLAN-CUR-101` through `PLAN-CUR-108` | `PARTIAL` | Every Linux lesson has recall, diagnostic, production, and transfer questions with model answers | Twenty-three legacy guides plus three structured `LES-0006` assessments exist; coverage by every Linux lesson, level, and difficulty remains incomplete |
...
| `PLAN-INT-006` | P0 | Rubrics and mastery separation | `PLAN-GOV-005`, `PLAN-INT-001` | `PARTIAL` | Scores reasoning, evidence, safety, trade-offs, communication, and independence; website score never auto-promotes competency | Nine assessment rubrics are authored and all three independent transfers, including `ASM-0009`, are answer-isolated; root reconciliation, ledger authorization, and complete evidence-lineage audits remain |
...
| `PLAN-QUA-002` | P0 | Content/schema validation | `PLAN-ARC-004` | `PARTIAL` | Schemas fail closed; IDs, legacy reservations, prerequisites, metadata, non-empty sections, answers, references, command assertions, paths, ownership, backlinks, and cycles validate | Current content passes for three lessons, nine assessments, and 24 references; schema passes 38 of 39 with one Windows `EPERM` skip. CI wiring remains |
...
| `PLAN-QUA-004` | P1 | Route, link, and 404 tests | `PLAN-WEB-002`, `PLAN-ARC-004` | `PARTIAL` | All declared routes return expected status; links and anchors resolve; invalid IDs return 404 | Fifteen routes, two invalid-route 404s, and 15 discovered internal links pass; comprehensive browser navigation remains unavailable |
...
| `PLAN-AUD-005` | P0 | Mastery-integrity audit | `PLAN-GOV-005`, `PLAN-INT-006` | `PARTIAL` | Reading, button clicks, localStorage, model answers, and mentor-generated output cannot be mistaken for learner evidence | Twenty-one reader tests cover LES-0008 migration, advisory prerequisites, explicit non-mastery language, and `ASM-0009` isolation; full evidence-lineage and learner-review workflow remain |
...
1. Author the next content-first batch as `LES-0009` through `LES-0013`, beginning with the next unused identity `LES-0009` and the dependency-ordered Volume 00 safety foundation: Ubuntu/WSL shell behavior, command risk, secrets, Git change discipline, rollback, and cleanup.
2. Keep LES-0008 at `substantive-draft`; complete ShellCheck, concurrency/race coverage, browser QA, formal acceptance, independent learner transfer, and delayed recall without inferring mastery from mentor project evidence.
...
4. Add search filters, internal crawling, and browser-level persistence, keyboard, clipboard, night, mobile, print, and visual checks across both routed volumes.
5. Restore Docker integration in Ubuntu, then run the full-envelope v2 lifecycle, one-field counterfeit refusals, check/reset paths, and exact legacy-v1 retirement.
6. Complete remaining host-lab failure matrices, ShellCheck, accessibility/privacy/performance checks, dependency/license review, CI wiring, and fresh-clone reproducibility before `PLAN-AUD-001`.
7. Keep the learner on `PLAN-CUR-101`/`PLAN-LAB-102` until learner-operated remediation evidence is reviewed; content availability and advisory prerequisite navigation must not bypass that gate.
