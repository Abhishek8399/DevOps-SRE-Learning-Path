...
| `PLAN-GOV-004` | P0 | Contribution and review workflow | `PLAN-GOV-003` | `PARTIAL` | A future human or AI can add content without chat history; safety and definition-of-done checks are explicit | LES-0006 exercised the authoring, validation, independent-review, and correction path; fresh-contributor dry-run remains |
...
| `PLAN-GOV-006` | P1 | Reference freshness policy | `PLAN-GOV-003` | `PARTIAL` | Version-sensitive claims identify primary sources, tested versions, review dates, and expiration/recheck rules | Eight primary LES-0006 records include version and review windows; repository-wide stale-reference report remains |
...
| `PLAN-ARC-004` | P1 | Structured Markdown content schema | `PLAN-ARC-002`, `PLAN-ARC-003` | `COMMITTED` | New content lives under `book/volumes/...`; strict JSON front matter covers metadata; diagrams, command cards, incidents, answers, lab links, assessments, references, and legacy identities have validated structures | 36 focused cases, live LES-0006 corpus, safe renderer, server registry, build, route, and answer-isolation validation |
| `PLAN-ARC-005` | P1 | Typed-to-structured-Markdown migration | `PLAN-ARC-004` | `PARTIAL` | Existing five lessons migrate without URL, ID, text, diagram, answer, lab, search, or device-state regression | Adapter and five-entry state migration are tested; migration of the five legacy lesson bodies remains |
...
| `PLAN-ARC-007` | P1 | Reference registry | `PLAN-GOV-006`, `PLAN-ARC-004` | `PARTIAL` | Primary references record title, canonical HTTPS URL, version/date, lesson IDs, review window, relevance, and paraphrase policy | Strict relationships plus eight live LES-0006 references pass; full-curriculum population and freshness report remain |
| `PLAN-ARC-008` | P2 | Content search index | `PLAN-ARC-002`, `PLAN-ARC-003` | `PARTIAL` | Local search finds terms, symptoms, commands, tools, and lesson IDs without an external service | Six-document production catalog and deterministic golden queries pass; filters, cross-volume scale, and disconnected-browser proof remain |
...
| `PLAN-WEB-002` | P0 | Routed book reader | `PLAN-ARC-001` | `PARTIAL` | Separate library, volume, lesson, and practice routes; breadcrumbs; valid static params; invalid lessons return 404 | Twelve declared routes and 404 pass with the sixth schema-backed lesson; full internal-link and manual navigation review remain |
...
| `PLAN-WEB-005` | P1 | Find and resume | `PLAN-ARC-008`, `PLAN-WEB-003` | `PARTIAL` | Search, stable deep links, recent location, and bookmarks work locally; no convenience state becomes mastery evidence | Six IDs and additive v1 state migration pass; browser restart, cross-tab, and disconnected interaction tests remain |
...
| `PLAN-QUA-002` | P0 | Content/schema validation | `PLAN-ARC-004` | `PARTIAL` | Schemas fail closed; IDs, legacy reservations, prerequisites, metadata, non-empty sections, answers, references, command assertions, paths, ownership, backlinks, and cycles validate | 36 dependency-free cases exercise the live corpus; 35 pass and one Windows symlink case skips; CI wiring remains |
...
| `PLAN-QUA-004` | P1 | Route, link, and 404 tests | `PLAN-WEB-002`, `PLAN-ARC-004` | `PARTIAL` | All declared routes return expected status; links and anchors resolve; invalid IDs return 404 | Twelve-route/404 smoke passes; automated internal crawl and browser navigation remain |
...
| `PLAN-AUD-005` | P0 | Mastery-integrity audit | `PLAN-GOV-005`, `PLAN-INT-006` | `PARTIAL` | Reading, button clicks, localStorage, model answers, and mentor-generated output cannot be mistaken for learner evidence | Seventeen reader tests include additive state migration and independent-answer isolation; full evidence-lineage review remains |
...
3. Add cross-volume search filters, internal crawling, and browser-level persistence, keyboard, clipboard, night, mobile, print, and visual checks.
