...
| `REL-DOC-002` | Only intended files changed | `git status --short`; full `git diff` review | `PASS` for checkpoint `24201bb` | Every feature change maps to a plan ID; no unrelated file or secret was staged |
...
| `REL-DOC-005` | Generated artifact hygiene | Inspect `tsconfig.tsbuildinfo` and build outputs before/after tests | `PASS` for checkpoint `24201bb` | `*.tsbuildinfo` remains ignored; feature validation left no unexplained generated source artifact |
...
| `REL-WEB-005` | Route matrix | Start loopback server; request `/`, `/book`, `/book/linux`, six lesson URLs, `/practice/storage`, `/search`, `/my-learning`, and an invalid ID | `PASS` | Twelve declared routes return 200 with one `h1` each; invalid lesson returns 404; no external executable/style asset |
...
| `REL-WEB-007` | Content completeness | Validate required lesson sections, glossary, decoders, answers, labs, and IDs | `PARTIAL` | LES-0006 validates 18 required sections, 3 diagrams, 12 read-only command cards, 1 guided lab, 2 incidents, 3 assessments, and 8 primary references; full chapter acceptance and remaining-curriculum audits remain |
...
| `REL-WEB-010` | Answer disclosure and isolation | Inspect storage panels and structured assessment projections | `PASS` with scoped browser limitation | Seven storage panels remain closed initially; LES-0006 exposes two complete answer guides while the independent transfer projection excludes answer fields. Browser interaction remains unverified |
| `REL-WEB-011` | Reader state, search, adapter, and isolation suite | `npm run test:reader` | `PASS` | Seventeen dependency-free tests cover schema recovery, storage failure/clear, transitions, recents, deterministic live-catalog search, additive state migration, structured parsing, and independent-answer isolation |
...
| `REL-SEC-001` | Secret and sensitive-data scan | Scan tracked/untracked source and full staged diff; manually review samples | `PASS` for checkpoint `24201bb`; Git-history scan not included | No credential, token, key, employer detail, internal URL, local identity path, or production evidence was found |
...
| `REL-SEC-003` | External-request/privacy audit | Browser/network capture during normal reading and interactions | `PARTIAL` | Twelve rendered routes contain no external executable/style assets; browser-network capture remains unavailable and unclaimed |
| `REL-SEC-004` | Loopback binding | Inspect listening socket during launch | `PASS` for current production smoke | Temporary server listened exactly on `127.0.0.1:4179` and stopped cleanly; fresh-clone launcher smoke remains separate |
...
| `REL-REP-003` | Git review | `git diff`, `git diff --check`, secret scan, generated-file check | `PASS` for checkpoint `24201bb` | Logical, reviewable, safe feature diff |
| `REL-REP-004` | Commit and push | Non-interactive commit/push after all required checks | `PASS` for checkpoint `24201bb`; final audited release pending | `main` and `origin/main` matched exactly after the non-force push; full commit ID is recorded in `VER-073` |
