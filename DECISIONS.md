...
### DEC-080 - Cost optimization is a reliability change and every cost claim needs semantic identity

**Decision:** `FIN-001` will trace every cost decision through `decision/owner -> data generator/export -> completeness/freshness -> periods/currency/cost meaning -> quantity/rate/mix -> ownership/pools -> allocation/conservation -> stable unit outcome -> forecast/range/budget -> anomaly classification -> optimization and authority -> SLO/security/recovery guardrails -> canary/rollback -> delayed financial reconciliation`. List, contracted, billed, effective and invoice cost remain distinct. Estimate, forecast, range, budget, actual, saving and avoidance remain distinct. Lower spend alone is not an accepted outcome.

**Rationale:** A backfill can look like a spike; missing data can look like a saving; tag changes can move ownership without changing the bill; failed requests can improve a weak unit metric; rate discounts can preserve waste; commitment coverage can hide vacancy; rightsizing can remove failure reserve; deletion can remove recovery evidence; and provider recommendations can omit product, contract and SLO context. Optimizing a dashboard number before proving semantics and downstream promises can trade visible spend for outages, security exposure, recovery failure or engineering toil.

**Consequences:** `LES-0081` remains quarantined despite a substantive manuscript, direct schemas, a guarded 64-case/63-gate/four-calculation no-cloud-authority model, complete Ubuntu 24.04 UID-1000 authority/root/unknown-artifact refusal and cleanup lifecycle, and canonical regressions passing. The model uses fictional data and performs no cloud, billing, Kubernetes, infrastructure, budget, purchase or optimization action. Representative sanitized export/telemetry evidence, current schema/price/contract review, technical/financial/security/instructional review, delayed recall and learner transfer remain required. No realized saving, production safety, publication acceptance or mastery is inferred.

### DEC-081 - Architecture begins with measurable outcomes and keeps models separate from evidence

**Decision:** `ARC-001` uses the SCALE loop: scope one accountable decision and user operation; calculate a unit-bound operating envelope; architect consistent C4, request, state, identity, trust and failure-domain views; locate overload, ambiguity, recovery, migration and adversarial risks; explain feasible alternatives, vetoes, sensitivity and residual ownership; then revise the model when validation evidence changes. Every acknowledgement must identify the obligation accepted, every important fact must have authority/conflict/reconciliation semantics, and every architecture claim must be labeled declared, calculated, tested or observed.

**Rationale:** Technology-first diagrams hide the user promise, mixed abstraction hides ownership, free-form quality words cannot be tested, asynchronous paths hide waiting and duplicates, timeout is often confused with failure, redundancy can share one failure domain, weighted scores can average away veto constraints, and an ADR can document a choice without proving implementation or behavior. Binding decisions to state transitions, units, evidence limits and revision triggers makes designs reviewable and incident-usable.

**Consequences:** `LES-0082` remains quarantined despite a 15,812-word manuscript, three assessments, sixteen sources, a guarded 67-case/66-gate/five-calculation Ubuntu lifecycle and passing canonical regressions. All service names, workloads, capacities, availability, latency, queues, recovery exposure, costs, risks and scores are fictional. The model performs no network, provider, runtime, deployment, fault, migration or production action. Representative sanitized architecture evidence, accepted ADRs, formal review, two independently scored unfamiliar transfers, delayed recall and publication remain required; repository validation does not award learner mastery.

### DEC-082 - Strategy is an evidence-governed decision system and migration is a temporary architecture

**Decision:** `ARC-002` translates executive intent into an owned mandate, versioned confidence-aware portfolio evidence, per-workload rationalization, outcome-linked principles and standards, feasible options, quantitative ranges and accountable decisions. Hard legal, security, correctness, recovery, export and exit constraints are applied before weighted preference. Migration is designed as a temporary source/bridge/target architecture with compatibility, one writer authority, synchronization checkpoints, cutover, tested state recovery, reconciliation, handover, benefit review and separately authorized decommission.

**Rationale:** A target diagram cannot establish current-state truth, a rationalization label cannot execute a transition, a weighted score can average away an infeasible supplier, bulk-copy time is not cutover time, route reversal cannot rewind state, and a project deadline does not authorize deletion. Preserving evidence class, unit, time, owner, expiry, decision version and recovery semantics makes strategy revisable and migration operable.

**Consequences:** `LES-0083` remains quarantined despite a 16,295-word manuscript, three assessments, eighteen sources, a guarded 71-case/70-gate/five-calculation Ubuntu lifecycle and passing canonical regressions. All portfolio, demand, transfer, cost, benefit, risk, supplier and score data are fictional. The model performs no discovery, provider, vendor, contract, infrastructure, data movement, cutover, deletion or production action. Representative sanitized evidence, accepted strategy/governance/vendor/migration decisions, formal multidisciplinary review, two independently scored unfamiliar transfers, delayed recall and publication remain required; project validation does not award learner mastery.

### DEC-083 - Operational documentation is a versioned evidence and decision system

**Decision:** `DOC-001` treats consequential technical writing as the path `reader/outcome -> canonical evidence and typed claims -> purpose-fit artifact and audience view -> scoped review -> representative rehearsal -> protected activation -> observed use -> trigger -> revision/supersession/archive`. Observed, calculated, declared, decided, inferred and unknown claims retain source, scope, time, unit, confidence, sensitivity and expiry. A document can guide an authorized action but never creates production or publication authority.

**Rationale:** Correct grammar cannot repair an unsupported material claim, an ambiguous target, hidden trust boundary, unsafe command, rewritten decision, contradictory audience update, blame-only postmortem or stale active route. Version control proves change history but not truth, approval, usability or authority. Keeping fact ownership, artifact lifecycle, decision state, incident state and procedure authority distinct makes the documentation system reviewable and recoverable.

**Consequences:** `LES-0084` remains quarantined despite a 15,009-word manuscript, three assessments, eighteen sources, a guarded 73-case/72-gate/five-calculation Ubuntu lifecycle and passing canonical regressions. All document packets, claims, timestamps, incidents, actions and audience views are fictional. The model performs no publication, message, ticket, incident-system, provider or runtime action. Representative sanitized organizational artifacts, accepted procedures and decisions, formal multidisciplinary review, independently scored unfamiliar transfer, delayed recall and publication remain required; project validation does not award learner mastery.

### DEC-084 - Technical leadership is an outcome, authority, capacity and learning control system

**Decision:** `LDR-001` teaches technical leadership as `outcome -> evidence/unknowns -> accountable decision and preserved objections -> bounded authority plus finite capacity -> delegated action -> observed effect -> learning`. Ownership means one reviewable closure path with sufficient authority and resources, not one person performing every task. Priority is an ordering decision that exposes reserve and stopped work. Delegation transfers bounded judgment with context, checkbacks and escalation. Psychological safety protects early truth while accountability retains explicit behavior and outcome standards.

**Rationale:** Title, confidence, activity, agreement, templates and numerical coverage can imitate leadership without producing reliable decisions. Engineering organizations fail when demand exceeds hidden capacity, responsibility moves without authority, expertise becomes a queue, dissent becomes unsafe, stakeholder views contradict, incident roles collapse or career narratives outrun evidence. A mechanism-first model makes these failure modes observable without claiming people or performance authority.

**Consequences:** `LES-0085` uses only fictional local data, refuses root and external people/messaging/ticket/calendar/runtime authority, and never evaluates an employee or sends a stakeholder message. Organization handbooks remain contextual examples rather than universal policy. Automated gates prove structure and arithmetic only; representative authorized decisions, human review, longitudinal outcomes, truthful learner evidence, independent transfer, delayed recall and formal promotion remain separate.

### DEC-085 - Interview readiness is truthful evidence retrieval and transfer, not script fluency or hiring prediction

**Decision:** `INT-002` teaches interview preparation as `versioned role -> competency/level hypothesis -> learner-owned permitted evidence -> typed canonical claims -> STAR-L/project/incident/architecture packet -> duration variant -> question-intent routing -> changing follow-up -> evidence audit -> revision -> delayed unfamiliar transfer`. Answer depth may change, but canonical facts, attribution, authority and confidentiality boundaries may not. Practice automation may structure fictional evidence and arithmetic; it never evaluates a candidate, predicts hiring or authorizes live assistance.

**Rationale:** Polished scripts, confident delivery, exact-looking metrics, product lists, self-scores and familiar prompt repetition can imitate readiness while hiding unsupported claims, borrowed team outcomes, retrospective causality, shallow state reasoning or policy violations. A claim ledger plus reviewer-controlled novelty makes contradictions and evidence gaps observable. Separating a practice receipt from an employer decision avoids false authority and keeps improvement actionable.

**Consequences:** `LES-0086` remains quarantined despite a 15,309-word manuscript, three assessments, eighteen sources, a guarded 73-case/72-gate/five-calculation Ubuntu lifecycle and passing canonical regressions. All roles, people, claims, metrics, incidents, projects, questions, answers and scores in the model are fictional. The lab refuses candidate/ATS/resume/recording/live-interview/AI-service/runtime authority and makes no external call. Truthful learner-owned evidence, employer-policy compliance, human review, two independent unfamiliar role loops, delayed transfer, formal multidisciplinary review and publication remain required; project validation does not award readiness, level, mastery or a hiring outcome.


### DEC-086 - A career roadmap is a versioned evidence and capacity decision system, not a title or tool checklist

**Decision:** `CAR-001` uses the path `versioned role -> work/responsibility requirements -> permitted learner evidence and proof limits -> explicit gaps -> prerequisite graph -> finite capacity and stopped work -> artifact/fault/recovery/review milestone -> delayed unfamiliar transfer -> versioned roadmap diff`. Shared systems, networking, software, delivery, reliability, security and documentation form a trunk; cloud/DevOps, SRE, platform, private-cloud, data-platform and architect work branch by actual state, failure and responsibility models. Titles, years, courses, credentials, repository counts and fixture scores never auto-assign level or predict hiring.

**Rationale:** Tool lists and aggressive calendars can appear comprehensive while violating dependencies and capacity. Course completion can be useful exposure yet remain brittle under unfamiliar failure. Local labs can provide strong bounded evidence yet cannot create production authority, organizational adoption or longitudinal outcomes. Explicit proof limits, reserve, failure-shaped acceptance and preserved revisions make the roadmap executable and truthful.

**Consequences:** `LES-0087` remains quarantined despite a 15,009-word manuscript, three assessments, eighteen sources, a guarded 73-case/72-gate/six-calculation Ubuntu lifecycle and passing canonical regressions. All roles, evidence, hours, milestones, reviews and scores in the model are fictional. The lab refuses learner/personnel/provider/production authority, level inference, hiring prediction and external calls. Learner-owned evidence and constraints, independent unfamiliar role mapping, delayed transfer, formal multidisciplinary review and publication remain required; project checks do not award competence, level, readiness, mastery or employment.

### DEC-087 - A service capstone is a versioned user-operation, state, evidence and recovery control system

**Decision:** `CAP-001` integrates one small evolving service through `user intent -> TLS/proxy -> application contract -> atomic durable state -> correlated telemetry -> immutable release identity -> bounded candidate -> SLI/correctness decision -> rollback or separate restore -> validation -> postmortem`. Liveness, readiness, correctness, SLO health and business outcome remain separate claims. Restart, executable rollback, failover and data restore remain separate recovery levers. A timed-out write is reconciled with the same idempotency identity; recovery never overwrites the only active state copy.

**Rationale:** A collection of disconnected tool demos can pass while no one can explain a failed user operation or recover it safely. Connecting source/config/schema/artifact identity to atomic state, low-cardinality evidence, progressive exposure and verified recovery makes cross-domain trade-offs observable. Keeping proof limits explicit prevents a local green pipeline, image digest, health check, backup command or synthetic load sample from becoming a production claim.

**Consequences:** `LES-0088` remains quarantined despite the verified Python/SQLite fixture, container/TLS/Prometheus topology, three assessments, twenty sources, 11,863-word manuscript and passing Ubuntu/Docker/canonical gates. The standard-library server, self-signed certificate, local database, inactive nested CI template, synthetic load and local recovery times are teaching evidence only. Fresh-clone reviewer-owned changed behavior and hidden fault, IaC/cluster integration, representative capacity/security/HA/RPO/RTO, formal multidisciplinary review, learner transfer, delayed recall and publication remain required; project checks do not award production experience or mastery.

### DEC-088 - A Kubernetes platform capstone is a product contract plus explicit state and controller ownership

**Decision:** `CAP-002` integrates `developer job -> versioned ServiceRequest -> fail-closed deterministic generation -> immutable Git content -> declared reconciliation owner -> Kubernetes identity/authorization/admission -> controllers/scheduler/kubelet -> readiness/EndpointSlice -> exact user operation`. Namespace, RBAC, admission, network, compute, storage, secrets, nodes and control plane remain separate tenancy layers. Git declarations, etcd/API objects, application data, external resources and artifacts/keys remain separate recovery classes. Field conflicts require an ownership decision; force is bounded to the fixture's declared generator field.

**Rationale:** A portal, YAML template, cluster or green reconciler can imitate a platform while hiding developer failure, unsafe defaults, mutable source, controller fights and incomplete recovery. One typed contract joined to source identity, policy denials, observed controller transitions, user-path validation and state-class recovery makes those boundaries inspectable. Native CEL avoids an extra webhook dependency in the local fixture. The small invoked reconciler exposes Git identity and managed-field mechanics without falsely claiming all four OpenGitOps principles.

**Consequences:** `LES-0089` remains quarantined despite a verified three-node kind runtime, twelve tests, three RBAC decisions, three admission denials, drift correction, failed-release rollback, 100/100 bounded local probes, declaration reconstruction, exact cleanup, three assessments, twenty sources and a 14,423-word manuscript. Shared-host kind nodes are not availability zones; accepted NetworkPolicy is not packet enforcement; Git is not etcd/application-data backup; local latency is not production capacity; and the fixture is not an organizational platform. Enforcing CNI, maintained GitOps, signed supply chain, real identity/secrets, etcd/data restore, staged upgrade, representative load/usability, independent hidden-fault transfer, formal review, publication and mastery remain separate gates.

### DEC-089 - A distributed-data capstone is explicit authority, identity, reconciliation and recovery

**Decision:** `CAP-003` integrates `write intent -> idempotency identity -> atomic order/outbox transaction -> retained event identity -> at-least-once relay -> partition -> inbox/effect transaction -> disposable cache -> reconciliation and lineage -> isolated restore -> retained replay`. PostgreSQL owns transactional truth, Kafka owns retained delivery positions, derived facts own analytical projections, Redis owns only rebuildable acceleration, and backup artifacts own a point-in-time recovery input. Duplicate delivery, incompatible events, skew, lag, freshness, correctness and recovery remain distinct states and claims.

**Rationale:** Green components do not prove a correct data journey. A producer can time out after publishing, a consumer can repeat work, one partition can be hot while total lag looks modest, a broker can be healthy while facts are missing, and a successful restore can still target the wrong state boundary. Stable identities, atomic transitions, privacy-safe quarantine, per-partition evidence, reconciliation and separate restore/replay make those failure windows observable and recoverable.

**Consequences:** `LES-0090` remains quarantined despite thirteen tests, three verified absent-to-absent fault/recovery cycles, three assessments, twenty sources and a 10,427-word manuscript. PostgreSQL, Kafka and Redis each run as one bounded local process; the teaching consumer is not a durable production group; no multi-broker/HA guarantee, authenticated transport, representative Spark/Flink/Iceberg workload, accepted scale/RPO/RTO, formal review, learner transfer or production result is inferred. Publication and mastery remain separate gates.

### DEC-090 - A private cloud is joined state authorities across physical failure domains, not one ACTIVE status

**Decision:** `CAP-004` follows `protected workload intent -> identity/policy -> quota/image trust -> Placement inventory and generation-bound allocation -> Nova cell lifecycle -> KVM/libvirt runtime -> Ceph-backed state -> Neutron/OVN intent -> chassis and gateway realization -> exact user operation`. Physical power/network domains, controller quorum, Placement allocations, Nova lifecycle, OVN intent, host flows, Ceph maps/data, guest runtime and BMC tasks remain distinct authorities. Recovery chooses restart, migrate, evacuate, rebuild, restore or reconcile only after the failed state class and writer are known.

**Rationale:** `ACTIVE` can coexist with a broken MTU, unreachable gateway, missing volume, stale allocation, incompatible migration target or unsafe storage headroom. Direct database edits can make one view green while increasing divergence. A failure-domain topology plus generation-aware capacity, explicit compatibility, rack-aware storage protection, user-path evidence and cross-authority reconciliation makes those gaps visible.

**Consequences:** The LES-0091 simulator is intentionally dependency-free and invokes no infrastructure. Its seventeen tests, twelve decisions, dossier and exact cleanup prove only deterministic reasoning over synthetic contracts. The 9,574-word manuscript, three assessments and twenty references teach and test the reasoning contract without proving real KVM isolation, OpenStack scheduling, OVN convergence, Ceph durability, BMC fencing, upgrade compatibility, performance, availability, recovery time, production readiness or learner mastery. Representative authorized component evidence, formal review, executed independent transfer and publication remain separate gates.

### DEC-091 - An AI incident assistant is an untrusted adviser behind evidence and authority boundaries

**Decision:** `CAP-005` follows `synthetic incident -> scope and sanitization -> authorized versioned retrieval -> untrusted candidate generation -> claim and citation verification -> typed proposal -> deterministic policy -> independent approval -> narrow effect -> outcome reconciliation -> privacy-aware audit`. The model or fixture owns no incident-command, authorization, approval or production authority. Runbooks and telemetry remain untrusted data. A separate kill switch disables generation and tools while a deterministic evidence checklist remains available.
...
### DEC-093 - Complete the teaching corpus before building elaborate labs

**Decision:** Canonical manuscript depth and publication order take priority over elaborate synthetic lab environments. Each lesson still includes practical Ubuntu/Linux commands, field-by-field decoding, expected branches, safety boundaries, proof limits, validation, and cleanup. Until the teaching corpus is complete, labs should be concise, read-only where practical, and runnable on an Ubuntu or Linux machine without Docker, Kubernetes, cloud accounts, or custom incident simulators unless a concept genuinely cannot be taught safely otherwise.
