...
**Consequences:** LES-0092 will default to an offline deterministic untrusted-generator fixture so security and evaluation cases are reproducible without a model download, credential or network service. That fixture can prove orchestration invariants only; it cannot prove real-model quality or production safety. An optional local-model adapter must use the same contracts and be evaluated separately. Implementation, assessments, manuscript, formal AI/security/SRE/privacy review, reviewer-owned transfer and publication remain separate gates.
