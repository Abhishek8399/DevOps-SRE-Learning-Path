...
guard(){
  ...
  [[ -z "${AWS_PROFILE:-}${AWS_ACCESS_KEY_ID:-}${AWS_SECRET_ACCESS_KEY:-}${AWS_SESSION_TOKEN:-}${AZURE_CLIENT_ID:-}${AZURE_CLIENT_SECRET:-}${GOOGLE_APPLICATION_CREDENTIALS:-}${KUBECONFIG:-}${DOCKER_HOST:-}${ATS_TOKEN:-}${HR_SYSTEM_TOKEN:-}${RESUME_PATH:-}${INTERVIEW_RECORDING_PATH:-}${LIVE_INTERVIEW_TOKEN:-}${OPENAI_API_KEY:-}${ANTHROPIC_API_KEY:-}${PRODUCTION_ENDPOINT:-}" ]] || die credential-private-data-or-external-authority
...
doctor(){
  ...
  printf 'doctor=pass network=none user=%s candidate_evaluation=none hiring_prediction=none external_calls=none\n' "$(id -u)"
}
...
status(){
  ...
  packet_id="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1]))["packet_id"])' "$STATE/packet.json")"
...
case "${1:-}" in
  ...
  list) [[ $# -eq 1 ]] || die arguments; run_model roadmap "$STATE/cases.json" ;;
  ...
  stories|claims|variants|coverage|followups) [[ $# -eq 1 ]] || die arguments; run_model "$1" "$STATE/packet.json" ;;
  ...
  help|-h|--help) printf '%s\n' 'Usage: bash lab.sh doctor|setup|status|list|show CASE|evaluate CASE|evaluate-all|roadmap|stories|claims|variants|coverage|followups|cleanup' ;;
