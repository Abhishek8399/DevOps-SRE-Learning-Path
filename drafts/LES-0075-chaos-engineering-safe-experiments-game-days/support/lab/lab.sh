...
guard(){
  ...
  if [[ -e "$STATE" ]]; then
    ...
    [[ "$(<"$SENTINEL")" == "LES-0075:$(id -u)" ]] || die sentinel
  fi
}
...
inventory(){
  ...
  while IFS= read -r -d '' item; do
    relative="${item#"$STATE"/}"
    case "$relative" in .les0075|cases.json) ;; *) die "unknown-artifact:$relative" ;; esac
...
cleanup(){
  guard; [[ -d "$STATE" ]] || { printf 'cleanup=pass absent=true\n'; return; }
...
help_text(){
  cat <<'EOF'
Usage: bash lab.sh doctor|setup|status|list|show CASE|evaluate CASE|evaluate-all|cleanup
This offline decision model injects no fault and performs no process signal,
load, network, container, Kubernetes, cloud, service, privilege or host change.
EOF
}
