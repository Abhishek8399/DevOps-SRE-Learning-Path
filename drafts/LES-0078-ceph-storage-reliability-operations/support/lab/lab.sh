...
SENTINEL="$STATE/.les0078"
die(){ printf 'lab=fail reason=%s\n' "$1" >&2; exit 1; }

guard(){
  [[ "$(id -u)" -ne 0 ]] || die root-refused
  command -v python3 >/dev/null 2>&1 || die python3-missing
  [[ -z "${CEPH_ARGS:-}${CEPH_CONF:-}${CEPH_KEYRING:-}${CEPH_ARGS_FILE:-}${RBD_IMAGE_PREFIX:-}${OS_AUTH_URL:-}${OS_TOKEN:-}${OS_PASSWORD:-}${OS_CLOUD:-}${AWS_PROFILE:-}${AWS_ACCESS_KEY_ID:-}${AZURE_CLIENT_SECRET:-}${GOOGLE_APPLICATION_CREDENTIALS:-}${KUBECONFIG:-}${DOCKER_HOST:-}${LIBVIRT_DEFAULT_URI:-}${VIRSH_DEFAULT_CONNECT_URI:-}" ]] || die credential-or-cluster-authority
  [[ ! -L "$STATE" ]] || die state-symlink
  if [[ -e "$STATE" ]]; then
    [[ -d "$STATE" && -O "$STATE" && -f "$SENTINEL" && ! -L "$SENTINEL" ]] || die state-ownership
    [[ "$(<"$SENTINEL")" == "LES-0078:$(id -u)" ]] || die sentinel
  fi
}
...
inventory(){
  ...
  while IFS= read -r -d '' item; do
    relative="${item#"$STATE"/}"
    case "$relative" in .les0078|cases.json) ;; *) die "unknown-artifact:$relative" ;; esac
...
inventory_tools(){
  ...
  printf 'inventory=observed architecture=%s environment=%s ceph=%s rados=%s rbd=%s python3=%s cluster_calls=none\n' \
    "$(uname -m)" "$environment" "$(present ceph)" "$(present rados)" "$(present rbd)" "$(present python3)"
}
doctor(){
  guard
  [[ -r "$FIXTURE" && -r "$MODEL" ]] || die source
  python3 "$MODEL" validate "$FIXTURE"
  printf 'doctor=pass network=none user=%s cluster_calls=none\n' "$(id -u)"
}
setup(){
  guard; [[ ! -e "$STATE" ]] || die state-exists
  (umask 077; mkdir -- "$STATE"; printf 'LES-0078:%s\n' "$(id -u)" >"$SENTINEL"; cp -- "$FIXTURE" "$STATE/cases.json")
...
status(){
  ...
  printf 'status=ready cases=%s state=%s cluster_calls=none\n' "$count" "$STATE"
}
