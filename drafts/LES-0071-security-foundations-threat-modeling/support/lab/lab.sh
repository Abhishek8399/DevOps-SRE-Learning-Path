...
guard(){
  ...
  if [[ -e "$STATE" ]]; then
    ...
    [[ "$(<"$SENTINEL")" == "LES-0071:$(id -u)" ]] || die sentinel
  fi
}
...
inventory(){
  ...
  while IFS= read -r -d '' item; do
    relative="${item#"$STATE"/}"
    case "$relative" in .les0071|cases.json) ;; *) die "unknown-artifact:$relative" ;; esac
...
cleanup(){
  guard
  [[ -e "$STATE" ]] || { printf 'cleanup=pass already_absent=true\n'; return; }
  inventory
