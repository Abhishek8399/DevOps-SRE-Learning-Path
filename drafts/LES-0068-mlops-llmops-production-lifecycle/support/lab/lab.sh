...
STATE="/tmp/reliability-atlas-les0068-mlops-$(id -u)"
SENTINEL="$STATE/.les0068"
...
guard(){
  ...
  if [[ -e "$STATE" ]]; then
    ...
    [[ "$(<"$SENTINEL")" == "LES-0068:$(id -u)" ]] || die sentinel
  fi
}
inventory(){
  local item relative
  while IFS= read -r -d '' item; do
    relative="${item#"$STATE"/}"
    case "$relative" in .les0068|cases.json) ;; *) die "unknown-artifact:$relative" ;; esac
