...
CURRENT_UID="$(id -u)"
STATE_PARENT="/tmp"
STATE_NAME="reliability-atlas-les0034-${CURRENT_UID}"
...
validate_state() {
  [[ "${STATE_DIR}" == "/tmp/reliability-atlas-les0034-${CURRENT_UID}" ]] || { printf 'refused=true reason=state-name-invalid\n' >&2; exit 78; }
...
setup() {
  ...
  python3 -c 'import json,sys; value={"schemaVersion":1,"lessonId":"LES-0034","uid":int(sys.argv[1]),"statePath":sys.argv[2],"caseId":"queue-collapse-v1"}; handle=open(sys.argv[3],"w",encoding="utf-8"); json.dump(value,handle,indent=2); handle.write("\n"); handle.close()' "${CURRENT_UID}" "${STATE_DIR}" "${candidate}/manifest.json"
...
run_case() {
  local case_name="${1:-}"
  [[ -n "${case_name}" ]] || { printf 'usage: bash lab.sh run <timeline|claims|graph|counterfactual|methods|actions|verification>\n' >&2; exit 64; }
  require_tools
  validate_static_inputs
...
cleanup() {
  ...
  [[ "${resolved}" == "/tmp/reliability-atlas-les0034-${CURRENT_UID}" ]] || { printf 'refused=true reason=cleanup-path-invalid\n' >&2; exit 78; }
