...
UID_NOW="$(id -u)"
STATE="/tmp/reliability-atlas-les0060-queues-streams-$UID_NOW"
SENTINEL="$STATE/.les0060-sentinel"
...
doctor(){
  ...
  [[ -z "${AWS_ACCESS_KEY_ID:-}${AZURE_CLIENT_SECRET:-}${GOOGLE_APPLICATION_CREDENTIALS:-}${KAFKA_BOOTSTRAP_SERVERS:-}${RABBITMQ_URL:-}${NATS_URL:-}${DATABASE_URL:-}${KUBECONFIG:-}" ]]||die credential
  command -v python3 >/dev/null||die python
  printf 'doctor=pass runtime=offline-queue-stream-model\n'
}
guard(){
  [[ "$STATE" == "/tmp/reliability-atlas-les0060-queues-streams-$UID_NOW" ]]||die unsafe-root
  [[ -d "$STATE" && ! -L "$STATE" ]]||die state
  [[ "$(stat -c %u "$STATE")" == "$UID_NOW" ]]||die owner
  [[ -f "$SENTINEL" && ! -L "$SENTINEL" && "$(<"$SENTINEL")" == "les0060:$UID_NOW" ]]||die sentinel
  while IFS= read -r -d '' entry; do
    [[ ! -L "$entry" ]]||die child-symlink
    case "$(basename -- "$entry")" in .les0060-sentinel|cases.json) ;; *) die unexpected-artifact;; esac
...
clear_unknown(){
  [[ "$STATE" == "/tmp/reliability-atlas-les0060-queues-streams-$UID_NOW" && -d "$STATE" && ! -L "$STATE" ]]||die unsafe-clear
