...
UID_NOW="$(id -u)"
STATE="/tmp/reliability-atlas-les0062-data-pipeline-$UID_NOW"
SENTINEL="$STATE/.les0062-sentinel"
...
doctor(){
  ...
  [[ -z "${AWS_ACCESS_KEY_ID:-}${AZURE_CLIENT_SECRET:-}${GOOGLE_APPLICATION_CREDENTIALS:-}${KAFKA_BOOTSTRAP_SERVERS:-}${DATABASE_URL:-}${KUBECONFIG:-}${SPARK_HOME:-}${FLINK_HOME:-}" ]]||die credential
  command -v python3 >/dev/null||die python
  printf 'doctor=pass runtime=offline-data-pipeline-model\n'
}
guard(){
  [[ "$STATE" == "/tmp/reliability-atlas-les0062-data-pipeline-$UID_NOW" ]]||die unsafe-root
  [[ -d "$STATE" && ! -L "$STATE" ]]||die state
  [[ "$(stat -c %u "$STATE")" == "$UID_NOW" ]]||die owner
  [[ -f "$SENTINEL" && ! -L "$SENTINEL" && "$(<"$SENTINEL")" == "les0062:$UID_NOW" ]]||die sentinel
  while IFS= read -r -d '' entry; do
    [[ ! -L "$entry" ]]||die child-symlink
    case "$(basename -- "$entry")" in .les0062-sentinel|cases.json) ;; *) die unexpected-artifact;; esac
...
clear_unknown(){
  [[ "$STATE" == "/tmp/reliability-atlas-les0062-data-pipeline-$UID_NOW" && -d "$STATE" && ! -L "$STATE" ]]||die unsafe-clear
